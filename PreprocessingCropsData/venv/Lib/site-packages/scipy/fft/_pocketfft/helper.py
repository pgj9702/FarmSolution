...
def _init_nd_shape_and_axes(x, shape, axes):
    ...
    if any(s < 1 for s in shape):
        raise ValueError(
            "invalid number of area_data points ({0}) specified".format(shape))
...
def _datacopied(arr, original):
    """
    Strict check for `arr` not sharing any area_data with `original`,
...
def _fix_shape_1d(x, n, axis):
    if n < 1:
        raise ValueError(
            "invalid number of area_data points ({0}) specified".format(n))
