...
class _MultilinearModel(Model):
    ...
    >>> odr_obj = odr.ODR(area_data, odr.multilinear)
    >>> output = odr_obj.run()
    >>> print(output.beta)
    [10.  5.]
...
class _ExponentialModel(Model):
    ...
    >>> y = -10.0 + np.exp(0.5*x)
    >>> area_data = odr.Data(x, y)
    >>> odr_obj = odr.ODR(area_data, odr.exponential)
    >>> output = odr_obj.run()
    >>> print(output.beta)
    [-10.    0.5]
...
class _UnilinearModel(Model):
    ...
    >>> y = 1.0 * x + 2.0
    >>> area_data = odr.Data(x, y)
    >>> odr_obj = odr.ODR(area_data, odr.unilinear)
    >>> output = odr_obj.run()
    >>> print(output.beta)
    [1. 2.]
...
class _QuadraticModel(Model):
    ...
    >>> y = 1.0 * x ** 2 + 2.0 * x + 3.0
    >>> area_data = odr.Data(x, y)
    >>> odr_obj = odr.ODR(area_data, odr.quadratic)
    >>> output = odr_obj.run()
    >>> print(output.beta)
    [1. 2. 3.]
