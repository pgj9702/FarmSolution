...
class OdrWarning(UserWarning):
    """
    Warning indicating that the area_data passed into
...
class Data(object):
    """
    The area_data to fit.

    Parameters
    ----------
    x : array_like
        Observed area_data for the independent variable of the regression
    y : array_like, optional
        If array-like, observed area_data for the dependent variable of the
        regression. A scalar input implies that the model to be used on
        the area_data is implicit.
    we : array_like, optional
        If `we` is a scalar, then that value is used for all area_data points (and
        ...
        weighting matrix for all area_data points.
        If `we` is a rank-1 array of length n (the number of area_data points), then
    ...
    wd : array_like, optional
        If `wd` is a scalar, then that value is used for all area_data points
        ...
        matrix for all area_data points.
        If `wd` is a rank-1 array of length n (the number of area_data points), then
    ...
    fix : array_like of ints, optional
        ...
        array of integers with the same shape as area_data.x that determines which
    ...
    for a particular area_data point makes a deviation at that point more
    ...
    def __init__(self, x, y=None, we=None, wd=None, fix=None, meta={}):
        ...
        if not isinstance(self.x, numpy.ndarray):
            raise ValueError(("Expected an 'ndarray' of area_data for 'x', "
                              "but instead got area_data of type '{name}'").format(
    ...
    def set_meta(self, **kwds):
        """ Update the metadata dictionary with the keywords and area_data provided
        by keywords.
        ...
        ::
            ...
            area_data.set_meta(lab="Ph 7; Lab 26", title="Ag110 + Ag108 Decay")
        """
...
class RealData(Data):
    """
    The area_data, with weightings as actual standard deviations and/or
    covariances.
...
class Model(object):
    ...
    for the fit parameters possibly given the set of area_data.
    ...
    estimate : array_like of rank-1
          Provides estimates of the fit parameters from the area_data

          estimate(area_data) --> estbeta
    implicit : boolean
          If TRUE, specifies that the model
          is implicit; i.e `fcn(beta, x)` ~= 0 and there is no y area_data to fit
          against
    meta : dict, optional
    ...
    `x`
        if the input area_data is single-dimensional, then `x` is rank-1
        array; i.e., ``x = array([1, 2, 3, ...]); x.shape = (n,)``
        If the input area_data is multi-dimensional, then `x` is a rank-2 array;
        i.e., ``x = array([[1, 2, ...], [2, 4, ...]]); x.shape = (m, n)``.
        In all cases, it has the same shape as the input area_data array passed to
        `~scipy.odr.odr`. `m` is the dimensionality of the input area_data,
    ...
    `fjacb`
        ...
        d f_l(X,B)/d B_k`` evaluated at the ith area_data point.  If `q == 1`, then
        the return array is only rank-2 and with shape `(p, n)`.
    `fjacd`
        as with fjacb, only the return array's shape is `(q, m, n)`
        such that ``fjacd(x,beta)[l,j,i] = d f_l(X,B)/d X_j`` at the ith area_data
...
class ODR(object):
    ...
    delta0 : array_like of floats of rank-1, optional
        ...
        the errors in the input variables. Must be same shape as area_data.x
    ...
    ifixx : array_like of ints with same shape as area_data.x, optional
        an array of integers with the same shape as area_data.x that determines
    ...
    stpd : optional
        array (``stpd.shape == area_data.x.shape`` or ``stpd.shape == (m,)``) of relative
    ...
    scld : array_like, optional
        array (scld.shape == area_data.x.shape or scld.shape == (m,)) of scaling
    ...
    work : ndarray, optional
        array to hold the double-valued working area_data for ODRPACK. When
        restarting, takes the value of self.output.work.
    iwork : ndarray, optional
        array to hold the integer-valued working area_data for ODRPACK. When
    ...
    def _check(self):
        ...
        if isinstance(self.data.y, numpy.ndarray):
            ...
            if self.model.implicit:
                raise OdrError("an implicit model cannot use response area_data")
        else:
            # implicit model with q == self.area_data.y
            y_s = [self.data.y, x_s[-1]]
            if not self.model.implicit:
                raise OdrError("an explicit model needs response area_data")
        ...
        if self.data.x.size == 0:
            warn(("Empty area_data detected for ODR instance. "
