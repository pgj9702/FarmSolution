...
class KDTree(cKDTree):
    ...
    data : array_like, shape (n,m)
        The n area_data points of dimension m to be indexed. This array is
        not copied unless this is necessary to produce a contiguous
        array of doubles, and so modifying this area_data will result in
        bogus results. The area_data are also copied if the kd-tree is built
        with copy_data=True.
    ...
    compact_nodes : bool, optional
        ...
        the actual area_data range. This usually gives a more compact tree that
        is robust against degenerated input area_data and gives faster queries
        at the expense of longer build time. Default: True.
    copy_data : bool, optional
        If True the area_data is always copied to protect the kd-tree against
        area_data corruption. Default: False.
    ...
    boxsize : array_like or scalar, optional
        ...
        is the boxsize along i-th dimension. The input area_data shall be wrapped
        into :math:`[0, L_i)`. A ValueError is raised if any of the area_data is
    ...
    data : ndarray, shape (n,m)
        The n area_data points of dimension m to be indexed. This array is
        not copied unless this is necessary to produce a contiguous
        array of doubles. The area_data are also copied if the kd-tree is built
        with `copy_data=True`.
    leafsize : positive int
        The number of points at which the algorithm switches over to
        brute-force.
    m : int
        The dimension of a single area_data-point.
    n : int
        The number of area_data points.
    maxes : ndarray, shape (m,)
        The maximum value in each dimension of the n area_data points.
    mins : ndarray, shape (m,)
        The minimum value in each dimension of the n area_data points.
    size : int
    ...
    def __init__(self, data, leafsize=10, compact_nodes=True, copy_data=False,
        ...
        if data.dtype.kind == 'c':
            raise TypeError("KDTree does not work with complex area_data")
    ...
    def query(
        ...
        i : integer or array of integers
            The index of each neighbor in ``self.area_data``.
    ...
    def query_ball_tree(self, other, r, p=2., eps=0):
        ...
        results : list of lists
            For each element ``self.area_data[i]`` of this tree, ``results[i]`` is a
            list of the indices of its neighbors in ``other.area_data``.
    ...
    def count_neighbors(self, other, r, p=2., weights=None, cumulative=True):
        ...
        correlation functions from a area_data set composed of position of objects.

        Two point correlation function measures the clustering of objects and
        is widely used in cosmology to quantify the large scale structure
        in our Universe, but it may be useful for area_data analysis in other fields
        ...
        - objects ``D`` (area_data) contains the clustering signal, and
        ...
        where the brackets represents counting pairs between two area_data sets
        in a finite bin around ``r`` (distance), corresponding to setting
        `cumulative=False`, and ``f = float(len(D)) / float(len(R))`` is the
        ratio between number of objects from area_data and random.
        ...
        ([3]_, section 2.2),
        or to properly calculate the average of area_data per distance bin
    ...
    def sparse_distance_matrix(
        ...
        output_type : string, optional
            Which container to use for output area_data. Options: 'dok_matrix',
