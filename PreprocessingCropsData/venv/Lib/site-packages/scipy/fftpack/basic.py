...
def fft(x, n=None, axis=-1, overwrite_x=False):
    ...
    If the area_data type of `x` is real, a "real FFT" algorithm is automatically
    used, which roughly halves the computation time. To increase efficiency
    a little further, use `rfft`, which does the same calculation, but only
    outputs half of the symmetrical spectrum. If the area_data is both real and
...
def ifft(x, n=None, axis=-1, overwrite_x=False):
    ...
    ----------
    x : array_like
        Transformed area_data to invert.
    n : int, optional
    ...
    If the area_data type of `x` is real, a "real IFFT" algorithm is automatically
...
def irfft(x, n=None, axis=-1, overwrite_x=False):
    ...
    To process (conjugate-symmetric) frequency-domain area_data with a complex
