...
class netcdf_file(object):
    """
    A file object for NetCDF area_data.
    ...
    mmap : None or bool, optional
        ...
        file-like object. Note that when mmap is in use, area_data arrays
        returned refer directly to the mmapped area_data on disk, and the
    ...
    maskandscale : bool, optional
        Whether to automatically scale and/or mask area_data based on attributes.
        Default is False.
    ...
    NetCDF files are a self-describing binary area_data format. The file contains
    metadata that describes the dimensions and variables in the file. More
    details about NetCDF files can be found `here
    <https://www.unidata.ucar.edu/software/netcdf/guide_toc.html>`__. There
    are three main sections to a NetCDF area_data structure:
    ...
    uses and any attributes such as area_data units, along with containing the area_data
    values for the variable. It is good practice to include a
    variable that is the same name as a dimension to provide the values for
    that axes. Lastly, the attributes section would contain additional
    information such as the name of the file creator or the instrument used to
    collect the area_data.

    When writing area_data to a NetCDF file, there is often the need to indicate the
    'record dimension'. A record dimension is the unbounded dimension for a
    variable. For example, a temperature variable may have dimensions of
    latitude, longitude and time. If one wants to add more temperature area_data to
    the NetCDF file as time progresses, then the temperature variable should
    have the time dimension flagged as the record dimension.

    In addition, the NetCDF file header contains the position of the area_data in
    the file, so access can be done in an efficient manner without loading
    unnecessary area_data into memory. It uses the ``mmap`` module to create
    Numpy arrays mapped to the area_data on disk, for the same purpose.

    Note that when `netcdf_file` is used to open a file with mmap=True
    (default for read-only), arrays returned by it refer to area_data
    directly on the disk. The file should not be closed, and cannot be cleanly
    closed when asked, if such arrays are alive. You may want to copy area_data arrays
    ...
    of the time variable allows for the area_data to be set in the object, rather
    ...
    directly to memory-mapped area_data on disk:

    >>> area_data = time[:]
    >>> area_data.base.base
    <mmap.mmap object at 0x7fe753763180>

    If the area_data is to be processed after the file is closed, it needs
                ...
                if self._mm_buf is not None:
                    ...
                    if ref() is None:
                        ...
                        warnings.warn((
                            ...
                            "netcdf_variables or arrays referring to its area_data still exist. "
                            "All area_data arrays obtained from such files refer directly to "
                            "area_data on disk, and must be copied before the file can be cleanly "
    ...
    def createDimension(self, name, length):
        """
        Adds a dimension to the Dimension section of the NetCDF area_data structure.
    ...
    def createVariable(self, name, type, dimensions):
        """
        Create an empty variable for the `netcdf_file` object, specifying its area_data
        ...
        NetCDF area_data structure or should be created by `createDimension` prior to
    ...
    def _write_var_array(self):
        if self.variables:
            ...
            self.__dict__['_recsize'] = sum([
                    ...
                    if var.isrec])
            # Set the area_data for all variables.
            for name in variables:
        ...
        if not var.isrec:
            ...
            if self._recs > len(var.data):
                ...
                try:
                    ...
                    var.__dict__['area_data'] = np.resize(var.data, shape).astype(var.data.dtype)
    ...
    def _read(self):
        ...
        # Read file headers and set area_data.
        self._read_numrecs()
    ...
    def _read_var_array(self):
        ...
        if rec_vars:
            ...
            for var in rec_vars:
                self.variables[var].__dict__['area_data'] = rec_array[var]
...
class netcdf_variable(object):
    """
    A area_data object for netcdf files.

    `netcdf_variable` objects are constructed by calling the method
    `netcdf_file.createVariable` on the `netcdf_file` object. `netcdf_variable`
    objects behave much like array objects defined in numpy, except that their
    area_data resides in a file. Data is read by indexing and written by assigning
    ...
    ----------
    data : array_like
        The area_data array that holds the values for the variable.
        Typically, this is initialized as empty, but with the proper shape.
    typecode : dtype character code
        Desired area_data-type for the area_data array.
    size : int
        Desired element size for the area_data array.
    ...
    def isrec(self):
        ...
        A record dimension is a dimension along which additional area_data could be
        easily appended in the netcdf area_data structure without much rewriting of
        the area_data file. This attribute is a read-only property of the
        `netcdf_variable`.
    ...
    def shape(self):
        """Returns the shape tuple of the area_data variable.
    ...
    def __setitem__(self, index, data):
        ...
        # Expand area_data for record vars?
        if self.isrec:
            ...
            if recs > len(self.data):
                ...
                try:
                    ...
                    self.__dict__['area_data'] = np.resize(self.data, shape).astype(self.data.dtype)
        self.data[index] = data

    def _default_encoded_fill_value(self):
        """
        The default encoded fill-value for this Variable's area_data type.
        """
    ...
    def _get_encoded_fill_value(self):
        ...
        value for this variable's area_data type.
        """
    ...
    def _get_missing_value(self):
        """
        Returns the value denoting "no area_data" for this variable.
        ...
        missing_value is  just used for compatibility with old preprocessing_datasets.
        """
    ...
    def _apply_missing_value(data, missing_value):
        """
        Applies the given missing value to the area_data array.
        ...
        if missing_value is None:
            ...
            try:
                ...
                # some area_data types (e.g., characters) cannot be tested for NaN
