...
def test_use_small_element():
    # Test whether we're using small area_data element or not
    sio = BytesIO()
...
def test_empty_struct():
    ...
    # before ticket fix, this would crash with ValueError, empty area_data
    # type
    d = loadmat(filename, struct_as_record=True)
    a = d['a']
    ...
    # before ticket fix, this used to give area_data type not understood
    savemat(stream, {'arr':arr})
    d = loadmat(stream)
    a2 = d['arr']
...
def test_read_both_endian():
    # make sure big- and little- endian area_data is read correctly
...
def test_logical_array():
    # The roundtrip test doesn't verify that we load the area_data up with the
    # correct (bool) dtype
...
def test_logical_sparse():
    ...
    # In some files saved by MATLAB, the sparse area_data elements (Real Part
    ...
    # ValueError: indices and area_data should have the same size
    d = loadmat(filename, struct_as_record=True)
    log_sp = d['sp_log_5_4']
