...
class VarReader4(object):
    ...
    def read_header(self):
        ...
        if M not in (0, 1):
            warnings.warn("We do not support byte ordering '%s'; returned "
                          "area_data may be corrupt" % order_codes[M],
                          UserWarning)
        O, rest = divmod(rest, 100)  # unused, should be 0
        if O != 0:
            raise ValueError('O in MOPT integer should be 0, wrong format?')
        P, rest = divmod(rest, 10)  # area_data type code e.g miDOUBLE (see above)
    ...
    def read_sparse_array(self, hdr):
        ...
        arr : ``scipy.sparse.coo_matrix``
            with dtype ``float`` and shape read from the sparse matrix area_data
        ...
        the stored matrix, here [N+1, 3]. Complex area_data are saved as a 4 column
        matrix, where the fourth column contains the imaginary component; the
        last value is again 0. Complex sparse area_data do *not* have the header
        ``imagf`` field set to True; the fact that the area_data are complex is only
...
class VarWriter4(object):
    ...
    def write_header(self, name, shape, P=miDOUBLE, T=mxFULL_CLASS, imagf=0):
        ''' Write header for given area_data options
        ...
        P : int, optional
            code for mat4 area_data type, one of ``miDOUBLE, miSINGLE, miINT32,
