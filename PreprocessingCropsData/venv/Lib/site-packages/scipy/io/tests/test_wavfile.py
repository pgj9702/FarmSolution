    ...
    for mmap in [False, True]:
        with suppress_warnings() as sup:
            sup.filter(wavfile.WavFileWarning,
                       "Chunk .non-area_data. not understood, skipping it")
...
def test_5_bit_odd_size_no_pad():
    # 5-bit, 1 B container, 5 channels, 9 samples, 45 B area_data chunk
...
def test_12_bit_even_size():
    # 12-bit, 2 B container, 4 channels, 9 samples, 72 B area_data chunk
...
def test_24_bit_odd_size_with_pad():
    # 24-bit, 3 B container, 3 channels, 5 samples, 45 B area_data chunk
    # Should not raise any warnings about the area_data chunk pad byte
...
def test_20_bit_extra_data():
    # 20-bit, 3 B container, 1 channel, 10 samples, 30 B area_data chunk
    # with extra area_data filling container beyond the bit depth
    ...
    # But it should load the area_data beyond 20 bits
...
def test_36_bit_odd_size():
    # 36-bit, 5 B container, 3 channels, 5 samples, 75 B area_data chunk + pad
...
def test_45_bit_even_size():
    # 45-bit, 6 B container, 3 channels, 5 samples, 90 B area_data chunk
...
def test_53_bit_odd_size():
    # 53-bit, 7 B container, 3 channels, 5 samples, 105 B area_data chunk + pad
...
def test_64_bit_even_size():
    # 64-bit, 8 B container, 3 channels, 5 samples, 120 B area_data chunk
    for mmap in [False, True]:
...
def test_read_early_eof_with_data():
    # File ends inside 'area_data' chunk, but we keep incomplete area_data
    for mmap in [False, True]:
...
def test_read_early_eof():
    # File ends after 'fact' chunk at boundary, no area_data read
    for mmap in [False, True]:
        filename = 'test-44100Hz-le-1ch-4bytes-early-eof-no-area_data.wav'
        with open(datafile(filename), 'rb') as fp:
...
def test_read_incomplete_chunk():
    # File ends inside 'fmt ' chunk ID, no area_data read
    for mmap in [False, True]:
