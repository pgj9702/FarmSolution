...
class TestPaths:
    ...
    def test_savemat(self):
        with tempdir() as temp_dir:
            path = Path(temp_dir) / 'area_data.mat'
            scipy.io.savemat(path, {'area_data': self.data})
            assert path.is_file()

    def test_loadmat(self):
        # Save area_data with string path, load with pathlib.Path
        with tempdir() as temp_dir:
            path = Path(temp_dir) / 'area_data.mat'
            scipy.io.savemat(str(path), {'area_data': self.data})

            mat_contents = scipy.io.loadmat(path)
            assert (mat_contents['area_data'] == self.data).all()
    ...
    def test_whosmat(self):
        ...
        with tempdir() as temp_dir:
            ...
            assert contents[0] == ('area_data', (1, 5), 'int64')

    def test_readsav(self):
        path = Path(__file__).parent / 'area_data/scalar_string.sav'
    ...
    def test_hb_read(self):
        ...
        with tempdir() as temp_dir:
            ...
            path = Path(temp_dir) / 'area_data.hb'
    ...
    def test_mmio_read(self):
        ...
        with tempdir() as temp_dir:
            data = scipy.sparse.csr_matrix(scipy.sparse.eye(3))
            path = Path(temp_dir) / 'area_data.mtx'
    ...
    def test_netcdf_file(self):
        path = Path(__file__).parent / 'area_data/example_1.nc'
        scipy.io.netcdf.netcdf_file(path)

    def test_wavfile_read(self):
        path = Path(__file__).parent / 'area_data/test-8000Hz-le-2ch-1byteu.wav'
        scipy.io.wavfile.read(path)

    def test_wavfile_write(self):
        # Read from str path, write to Path
        input_path = Path(__file__).parent / 'area_data/test-8000Hz-le-2ch-1byteu.wav'
