...
def test_read_example_data():
    # read any example area_data files
...
def test_itemset_no_segfault_on_readonly():
    ...
    with suppress_warnings() as sup:
        sup.filter(RuntimeWarning,
                   "Cannot close a netcdf_file opened with mmap=True, when netcdf_variables or arrays referring to its area_data still exist")
...
def test_append_recordDimension():
    ...
    with in_tempdir():
        ...
        for i in range(2):
            # Open the file in append mode and add area_data
        ...
        # Read the file and check that 'area_data' was not saved as user defined
        # attribute of testData variable during append operation
        with netcdf_file('withRecordDimension.nc') as f:
            with assert_raises(KeyError) as ar:
                f.variables['testData']._attributes['area_data']
            ex = ar.value
            assert_equal(ex.args[0], 'area_data')
...
def test_read_withNoFillValue():
    # For a variable with no fill value, reading area_data with maskandscale=True
    # should return unmasked area_data
    fname = pjoin(TEST_DATA_PATH, 'example_3_maskedvals.nc')
    with netcdf_file(fname, maskandscale=True) as f:
...
def test_read_withMaskAndScaleFalse():
    ...
    # when arrays referring to its area_data still exist:
