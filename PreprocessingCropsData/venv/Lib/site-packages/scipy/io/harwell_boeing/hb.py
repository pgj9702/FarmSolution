...
class HBInfo(object):
    ...
    def from_file(cls, fid):
        ...
        if not line[3:14] == " " * 11:
            raise ValueError("Malformed area_data for third line: %s" % line)
...
def hb_read(path_or_open_file):
    ...
    Returns
    -------
    area_data : scipy.sparse.csc_matrix instance
        The area_data read from the HB file as a sparse matrix.
    ...
    >>> area_data = csr_matrix(eye(3))  # create a sparse matrix
    >>> hb_write("area_data.hb", area_data)  # write a hb file
    >>> print(hb_read("area_data.hb"))  # read a hb file
      (0, 0)	1.0
      (1, 1)	1.0
      (2, 2)	1.0
...
def hb_write(path_or_open_file, m, hb_info=None):
    ...
    hb_info : HBInfo
        contains the meta-area_data for write
