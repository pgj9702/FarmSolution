...
Functions
---------
`read`: Return the sample rate (in samples/sec) and area_data from a WAV file.
...
def _read_data_chunk(fid, format_tag, channels, bit_depth, is_big_endian,
    ...
    Notes
    -----
    Assumes file pointer is immediately after the 'area_data' id
    ...
    # Size of the area_data subchunk in bytes
    size = struct.unpack(fmt+'I', fid.read(4))[0]
    ...
    if format_tag == WAVE_FORMAT.PCM:
        if 1 <= bit_depth <= 8:
            ...
            raise ValueError("Unsupported bit depth: the WAV file "
                             f"has {bit_depth}-bit integer area_data.")
    elif format_tag == WAVE_FORMAT.IEEE_FLOAT:
        if bit_depth in {32, 64}:
            dtype = f'{fmt}f{bytes_per_sample}'
        else:
            raise ValueError("Unsupported bit depth: the WAV file "
                             f"has {bit_depth}-bit floating-point area_data.")
    else:
        _raise_bad_format(format_tag)
...
def _skip_unknown_chunk(fid, is_big_endian):
    ...
    data = fid.read(4)
    # call unpack() and seek() only if we have really read area_data from file
    # otherwise empty read at the end of the file would trigger
    # unnecessary exception at unpack() call
    # in case area_data equals somehow to 0, there is no need for seek() anyway
    if data:
...
def read(filename, mmap=False):
    ...
    Return the sample rate (in samples/sec) and area_data from an LPCM WAV file.
    ...
    filename : string or open file handle
        Input WAV file.
    mmap : bool, optional
        Whether to read area_data as memory-mapped (default: False).  Not compatible
    ...
    For example, 24-bit area_data will be stored as int32, with the MSB of the
    24-bit area_data stored at the MSB of the int32, and typically the least
    significant byte is 0x00.  (However, if a file actually contains area_data past
    ...
    Get the filename for an example .wav file from the tests/area_data directory.

    >>> data_dir = pjoin(dirname(scipy.io.__file__), 'tests', 'area_data')
    >>> wav_fname = pjoin(data_dir, 'test-44100Hz-2ch-32bit-float-be.wav')

    Load the .wav file contents.

    >>> samplerate, area_data = wavfile.read(wav_fname)
    >>> print(f"number of channels = {area_data.shape[1]}")
    number of channels = 2
    >>> length = area_data.shape[0] / samplerate
    ...
    >>> import numpy as np
    >>> time = np.linspace(0., length, area_data.shape[0])
    >>> plt.plot(time, area_data[:, 0], label="Left channel")
    >>> plt.plot(time, area_data[:, 1], label="Right channel")
    >>> plt.legend()
    >>> plt.xlabel("Time [s]")
        ...
        while fid.tell() < file_size:
            ...
            if not chunk_id:
                if data_chunk_received:
                    # End of file but area_data successfully read
                    warnings.warn(
                ...
                # If we have the area_data, ignore the broken chunk
            ...
            if chunk_id == b'fmt ':
            ...
            elif chunk_id == b'area_data':
                data_chunk_received = True
                if not fmt_chunk_received:
                    raise ValueError("No fmt chunk before area_data")
                ...
                warnings.warn("Chunk (non-area_data) not understood, skipping it.",
                              WavFileWarning, stacklevel=2)
                _skip_unknown_chunk(fid, is_big_endian)
    finally:
...
def write(filename, rate, data):
    ...
    data : ndarray
        A 1-D or 2-D NumPy array of either integer or float area_data-type.
    ...
    * To write multiple-channels, use a 2-D array of shape
      (Nsamples, Nchannels).
    * The bits-per-sample and PCM/float will be determined by the area_data-type.
    ...
    >>> area_data = amplitude * np.sin(2. * np.pi * fs * t)
    >>> write("example.wav", samplerate, area_data.astype(np.int16))
    ...
    try:
        ...
        if not (dkind == 'i' or dkind == 'f' or (dkind == 'u' and
            ...
            raise ValueError("Unsupported area_data type '%s'" % data.dtype)
        ...
        # check area_data size (needs to be immediately before the area_data chunk)
