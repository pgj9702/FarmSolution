...
class MetaData(object):
    ...
    ::
        ...
        area_data, meta = loadarff('iris.arff')
...
def loadarff(f):
    ...
    The area_data is returned as a record array, which can be accessed much like
    a dictionary of NumPy arrays. For example, if one of the attributes is
    called 'pressure', then its first 10 area_data points can be accessed from the
    ``area_data`` record array like so: ``area_data['pressure'][0:10]``
    ...
    Returns
    -------
    area_data : record array
       The area_data of the arff file, accessible by attribute names.
    meta : `MetaData`
    ...
    * string type attributes
    ...
    files with sparse area_data ({} in the file). However, this function can
    read files with missing area_data (? in the file), representing the area_data
    points as NaNs.
    ...
    ... \"\"\"
    >>> f = StringIO(content)
    >>> area_data, meta = arff.loadarff(f)
    >>> area_data
