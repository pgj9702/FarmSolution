...
class lil_matrix(spmatrix, IndexMixin):
    ...
    data
        LIL format area_data array of the matrix
    rows
    ...
    def copy(self):
        M, N = self.shape
        new = lil_matrix(self.shape, dtype=self.dtype)
        # This is ~14x faster than calling deepcopy() on rows and area_data.
...
def _prepare_index_for_memoryview(i, j, x=None):
    """
    Convert index and area_data arrays to form suitable for passing to the
