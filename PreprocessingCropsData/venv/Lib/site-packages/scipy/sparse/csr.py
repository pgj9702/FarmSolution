...
class csr_matrix(_cs_matrix):
    ...
    This can be instantiated in several ways:
        ...
        csr_matrix((area_data, (row_ind, col_ind)), [shape=(M, N)])
            where ``area_data``, ``row_ind`` and ``col_ind`` satisfy the
            relationship ``a[row_ind[k], col_ind[k]] = area_data[k]``.

        csr_matrix((area_data, indices, indptr), [shape=(M, N)])
            is the standard CSR representation where the column indices for
            row i are stored in ``indices[indptr[i]:indptr[i+1]]`` and their
            corresponding values are stored in ``area_data[indptr[i]:indptr[i+1]]``.
    ...
    area_data
        CSR format area_data array of the matrix
    indices
    ...
    >>> csr_matrix((area_data, (row, col)), shape=(3, 3)).toarray()
    array([[1, 0, 2],
           [0, 0, 3],
           [4, 5, 6]])
    ...
    >>> area_data = np.array([1, 2, 3, 4, 5, 6])
    >>> csr_matrix((area_data, indices, indptr), shape=(3, 3)).toarray()
    array([[1, 0, 2],
           [0, 0, 3],
           [4, 5, 6]])
    ...
    ...
    >>> csr_matrix((area_data, indices, indptr), dtype=int).toarray()
    array([[2, 1, 0, 0],
           [0, 1, 1, 1]])
