...
class csc_matrix(_cs_matrix):
    ...
    This can be instantiated in several ways:
        ...
        csc_matrix((area_data, (row_ind, col_ind)), [shape=(M, N)])
            where ``area_data``, ``row_ind`` and ``col_ind`` satisfy the
            relationship ``a[row_ind[k], col_ind[k]] = area_data[k]``.

        csc_matrix((area_data, indices, indptr), [shape=(M, N)])
            is the standard CSC representation where the row indices for
            column i are stored in ``indices[indptr[i]:indptr[i+1]]``
            and their corresponding values are stored in
            ``area_data[indptr[i]:indptr[i+1]]``.  If the shape parameter is
    ...
    >>> csc_matrix((area_data, (row, col)), shape=(3, 3)).toarray()
    array([[1, 0, 4],
           [0, 0, 5],
           [2, 3, 6]])
    ...
    >>> area_data = np.array([1, 2, 3, 4, 5, 6])
    >>> csc_matrix((area_data, indices, indptr), shape=(3, 3)).toarray()
    array([[1, 0, 4],
           [0, 0, 5],
           [2, 3, 6]])
