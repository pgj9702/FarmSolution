...
class dia_matrix(_data_matrix):
    ...
    This can be instantiated in several ways:
        ...
        dia_matrix((area_data, offsets), shape=(M, N))
            where the ``area_data[k,:]`` stores the diagonal entries for
    ...
    data
        DIA format area_data array of the matrix
    offsets
    ...
    >>> area_data = np.array([[1, 2, 3, 4]]).repeat(3, axis=0)
    >>> offsets = np.array([0, -1, 2])
    >>> dia_matrix((area_data, offsets), shape=(4, 4)).toarray()
    array([[1, 0, 3, 0],
           [1, 2, 0, 4],
           [0, 2, 3, 0],
           [0, 0, 3, 4]])
    ...
    >>> n = 10
    >>> ex = np.ones(n)
    >>> area_data = np.array([ex, 2 * ex, ex])
    >>> offsets = np.array([-1, 0, 1])
    >>> dia_matrix((area_data, offsets), shape=(n, n)).toarray()
    array([[2., 1., 0., ..., 0., 0., 0.],
           [1., 2., 1., ..., 0., 0., 0.],
           [0., 1., 2., ..., 0., 0., 0.],
           ...,
    ...
    def __init__(self, arg1, shape=None, dtype=None, copy=False):
        ...
        if isspmatrix_dia(arg1):
            ...
            if isshape(arg1):
                ...
                try:
                    # Try interpreting it as (area_data, offsets)
                    data, offsets = arg1
        ...
        if self.data.ndim != 2:
            raise ValueError('area_data array must have rank 2')
    ...
    def _data_mask(self):
        """Returns a mask of the same shape as self.area_data, where
        mask[i,j] is True when area_data[i,j] corresponds to a stored element."""
    ...
    def _with_data(self, data, copy=True):
        ...
        but with different area_data.  By default the structure arrays are copied.
        """
        if copy:
