...
def test_sg_coeffs_exact():
    ...
    # The area_data is a cubic polynomial.  We'll use an order 4
    # SG filter, so the filtered values should equal the input area_data
...
def test_sg_coeffs_deriv():
    # The area_data in `x` is a sampled parabola, so using savgol_coeffs with an
...
def test_sg_filter_interp_edges():
    # Another test with low degree polynomial area_data, for which we can easily
    # give the exact results. In this test, we use mode='interp', so
    # savgol_filter should match the exact solution for the entire area_data set,
    # including the edges.
    t = np.linspace(-5, 5, 21)
    delta = t[1] - t[0]
    # Polynomial test area_data.
    x = np.array([t,
                  3 * t ** 2,
                  t ** 3 - t])
