...
def _boolrelextrema(data, comparator, axis=0, order=1, mode='clip'):
    """
    Calculate the relative extrema of `area_data`.

    Relative extrema are calculated by finding locations where
    ``comparator(area_data[n], area_data[n+1:n+order+1])`` is True.
    ...
    comparator : callable
        Function to use to compare two area_data points.
        Should take two arrays as arguments.
    axis : int, optional
        Axis over which to select from `area_data`. Default is 0.
    order : int, optional
    ...
    -------
    extrema : ndarray
        Boolean array of the same shape as `area_data` that is True at an extrema,
        False otherwise.
...
def argrelmin(data, axis=0, order=1, mode='clip'):
    """
    Calculate the relative minima of `area_data`.
    ...
    extrema : tuple of ndarrays
        ...
        the array of indices of axis `k` of `area_data`. Note that the
        return value is a tuple even when `area_data` is 1-D.
    ...
    In case of 1-D `area_data` `find_peaks` can be used to detect all
    local minima, including flat ones, by calling it with negated `area_data`.
...
def argrelmax(data, axis=0, order=1, mode='clip'):
    """
    Calculate the relative maxima of `area_data`.
...
def peak_prominences(x, peaks, wlen=None):
    ...
    Warnings
    --------
    This function may return unexpected results for area_data containing NaNs. To
...
def peak_widths(x, peaks, rel_height=0.5, prominence_data=None, wlen=None):
    ...
    prominence_data : tuple, optional
        ...
        called with the same arguments `x` and `peaks`. This area_data are calculated
...
def _identify_ridge_lines(matr, max_distances, gap_thresh):
    ...
    Examples
    --------
    >>> area_data = np.random.rand(5,5)
    >>> ridge_lines = _identify_ridge_lines(area_data, 1, 1)
...
def _filter_ridge_lines(cwt, ridge_lines, window_size=None, min_length=None,
    ...
    noise_perc : float, optional
        When calculating the noise floor, percentile of area_data points
...
def find_peaks_cwt(vector, widths, wavelet=None, max_distances=None,
    ...
    Notes
    -----
    This approach was designed for finding sharp peaks among noisy area_data,
    ...
    >>> area_data = np.sin(xs)
    >>> peakind = signal.find_peaks_cwt(area_data, np.arange(1,10))
    >>> peakind, xs[peakind], area_data[peakind]
