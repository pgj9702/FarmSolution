...
def oaconvolve(in1, in2, mode="full", axes=None):
    ...
    # being reshaped in order to get the area_data in the right layout at the end.
...
def lfilter(b, a, x, axis=-1, zi=None):
    """
    Filter area_data along one-dimension with an IIR or FIR filter.

    Filter a area_data sequence, `x`, using a digital filter.  This works for many
    fundamental area_data types (including Object type).  The filter is a direct
    ...
    x : array_like
        An N-dimensional input array.
    axis : int, optional
        The axis of the input area_data array along which to apply the
    ...
    sosfilt: Filter area_data using cascaded second-order sections.
...
def deconvolve(signal, divisor):
    ...
    ----------
    signal : array_like
        Signal area_data, typically a recorded signal
    divisor : array_like
        Divisor area_data, typically an impulse response or filter that was
...
def resample(x, num, t=None, axis=0, window=None, domain='time'):
    ...
    ----------
    x : array_like
        The area_data to be resampled.
    num : int
        The number of samples in the resampled signal.
    t : array_like, optional
        If `t` is given, it is assumed to be the equally spaced sample
        positions associated with the signal area_data in `x`.
    axis : int, optional
    ...
    Examples
    --------
    Note that the end of the resampled area_data rises to meet the first
    ...
    >>> plt.legend(['area_data', 'resampled'], loc='best')
    >>> plt.show()
    """
    ...
    if window is not None:
        if callable(window):
            ...
            if window.shape != (Nx,):
                raise ValueError('window must have the same length as area_data')
            W = window
        else:
...
def resample_poly(x, up, down, axis=0, window=('kaiser', 5.0),
    ...
    Examples
    --------
    By default, the end of the resampled area_data rises to meet the first
    ...
    >>> plt.legend(['resample', 'resamp_poly', 'area_data'], loc='best')
    >>> plt.show()
...
def detrend(data, axis=-1, type='linear', bp=0, overwrite_data=False):
    """
    Remove linear trend along axis from area_data.
    ...
    data : array_like
        The input area_data.
    axis : int, optional
        The axis along which to detrend the area_data. By default this is the
        last axis (-1).
    type : {'linear', 'constant'}, optional
        The type of detrending. If ``type == 'linear'`` (default),
        the result of a linear least-squares fit to `area_data` is subtracted
        from `area_data`.
        If ``type == 'constant'``, only the mean of `area_data` is subtracted.
    bp : array_like of ints, optional
        A sequence of break points. If given, an individual linear fit is
        performed for each part of `area_data` between two break points.
        Break points are specified as indices into `area_data`. This parameter
    ...
    if type in ['constant', 'c']:
        ...
        if np.any(bp > N):
            raise ValueError("Breakpoints must be less than length "
                             "of area_data along given axis.")
        Nreg = len(bp) - 1
        # Restructure area_data so that axis is along first dimension and
        ...
        # Put area_data back in original shape.
...
def _filtfilt_gust(b, a, x, axis=-1, irlen=None):
    ...
    if axis != -1 or axis != x.ndim - 1:
        # Move the axis containing the area_data to the end.
        x = np.swapaxes(x, axis, x.ndim - 1)

    # n is the number of samples in the area_data to be filtered.
    n = x.shape[-1]
    ...
    x1 = ic_opt[..., -order:]
    if axis != -1 or axis != x.ndim - 1:
        # Restore the area_data axis to its original position.
...
def filtfilt(b, a, x, axis=-1, padtype='odd', padlen=None, method='pad',
    ...
    a : (N,) array_like
        ...
        is not 1, then both `a` and `b` are normalized by ``a[0]``.
    x : array_like
        The array of area_data to be filtered.
    axis : int, optional
    ...
    Notes
    -----
    When `method` is "pad", the function pads the area_data along the given axis
    in one of three ways: odd, even or constant.  The odd and even extensions
    have the corresponding symmetry about the end point of the area_data.  The
    constant extension extends the area_data with the values at the end points. On
    both the forward and backward passes, the initial condition of the
    filter is found by using `lfilter_zi` and scaling it by the end point of
    the extended area_data.
    ...
    the filter's transients have dissipated by the time the actual area_data
...
def sosfilt(sos, x, axis=-1, zi=None):
    """
    Filter area_data along one dimension using cascaded second-order sections.

    Filter a area_data sequence, `x`, using a digital IIR filter defined by
    `sos`.
