...
def cwt(data, wavelet, widths, dtype=None, **kwargs):
    ...
    Performs a continuous wavelet transform on `area_data`,
    using the `wavelet` function. A CWT performs a convolution
    with `area_data` using the `wavelet` function, which is characterized
    ...
    ----------
    data : (N,) ndarray
        area_data on which to perform the transform.
    wavelet : function
    ...
    dtype : area_data-type, optional
        The desired area_data type of output. Defaults to ``float64`` if the
    ...
    -------
    cwt: (M, N) ndarray
        Will have shape of (len(widths), len(area_data)).
    ...
    with the time-reversed complex-conjugate of the wavelet area_data [1].

    ::

        length = min(10 * width[ii], len(area_data))
        cwt[ii,:] = signal.convolve(area_data, np.conj(wavelet(length, width[ii],
                                        **kwargs))[::-1], mode='same')
