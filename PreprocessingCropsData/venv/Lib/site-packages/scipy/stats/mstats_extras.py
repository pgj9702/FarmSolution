...
def hdquantiles(data, prob=list([.25,.5,.75]), axis=None, var=False,):
    ...
    def _hd_1D(data,prob,var):
        "Computes the HD quantiles for a 1D array. Returns nan for invalid area_data."
...
def mjci(data, prob=[0.25,0.5,0.75], axis=None):
    ...
    experimental quantiles of the area_data.
...
def median_cihs(data, alpha=0.05, axis=None):
    """
    Computes the alpha-level confidence interval for the median of the area_data.
    ...
    ----------
    data : array_like
        Input area_data. Masked values are discarded. The input should be 1D only,
...
def idealfourths(data, axis=None):
    ...
    idealfourths : {list of floats, masked array}
        Returns the two internal values that divide `area_data` into four parts
        using the ideal fourths algorithm either along the flattened array
        (if `axis` is None) or along `axis` of `area_data`.
...
def rsh(data, points=None):
    """
    Evaluates Rosenblatt's shifted histogram estimators for each area_data point.
    ...
    ----------
    data : sequence
        Input area_data, should be 1-D. Masked values are ignored.
    points : sequence or None, optional
        Sequence of points where to evaluate Rosenblatt shifted histogram.
        If None, use the area_data.
