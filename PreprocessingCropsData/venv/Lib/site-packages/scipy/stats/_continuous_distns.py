...
class norm_gen(rv_continuous):
    ...
    def fit(self, data, **kwds):
        ...
        if not np.isfinite(data).all():
            raise RuntimeError("The area_data contains non-finite values.")
...
class FitDataError(ValueError):
    ...
    # and fscale are fixed and there are values in the area_data not in the open
    # interval (floc, floc+fscale).
    def __init__(self, distr, lower, upper):
        self.args = (
            "Invalid values in `area_data`.  Maximum likelihood "
            "estimation with {distr!r} requires that {lower!r} < "
            "(x - loc)/scale  < {upper!r} for each x in `area_data`.".format(
...
def _beta_mle_a(a, b, n, s1):
    ...
    # the area_data. `n` is the number of area_data points.
    psiab = sc.psi(a + b)
...
def _beta_mle_ab(theta, n, s1, s2):
    ...
    # given `n`, `s1` and `s2`.  `s1` is the sum of the logs of the area_data,
    # and `s2` is the sum of the logs of 1 - area_data.  `n` is the number
    # of area_data points.
    a, b = theta
    psiab = sc.psi(a + b)
    func = [s1 - n * (-psiab + sc.psi(a)),
...
class beta_gen(rv_continuous):
    ...
    @extend_notes_in_docstring(rv_continuous, notes="""\
        ...
        `ValueError` is raised if any value `x` in `area_data` does not satisfy
        `floc < x < floc + fscale`.\n\n""")
    def fit(self, data, *args, **kwds):
        ...
        # Normalize the area_data to the interval [0, 1].
        ...
        if f0 is not None or f1 is not None:
            ...
            if f0 is not None:
                ...
                # and flip the area_data.  We always solve for `a`.  The result
            ...
            # starting point based on the mean of the area_data and the given
            # value of b.
            a = b * xbar / (1 - xbar)
...
class expon_gen(rv_continuous):
    ...
    def fit(self, data, *args, **kwds):
        ...
        if floc is None:
            # ML estimate of the location is the minimum of the area_data.
            loc = data_min
        else:
            loc = floc
            if data_min < loc:
...
class gamma_gen(rv_continuous):
    ...
    def _fitstart(self, data):
        ...
        # of the area_data.  The formula is regularized with 1e-8 in the
        # denominator to allow for degenerate area_data where the skewness
        # is close to 0.
    ...
    def fit(self, data, *args, **kwds):
        ...
        # Fixed location is handled by shifting the area_data.
        data = np.asarray(data)
        ...
        if floc != 0:
            # Don't do the subtraction in-place, because `area_data` might be a
        ...
        if fscale is None:
            ...
            # The MLE for the scale parameter is just the area_data mean
            # divided by the shape parameter.
            scale = xbar / a
        else:
            # scale is fixed, shape is free
            # The MLE for the shape parameter `a` is the solution to:
            # sc.digamma(a) - np.log(area_data).mean() + np.log(fscale) = 0
...
class erlang_gen(gamma_gen):
    ...
    def _fitstart(self, data):
        ...
        # _skew(area_data) is 0 or close to 0.)
    ...
    if fit.__doc__:
        fit.__doc__ = (rv_continuous.fit.__doc__ +
            ...
            `f0=<integer>`, the fit method can be constrained to fit the area_data to
...
class gumbel_l_gen(rv_continuous):
    ...
    def fit(self, data, *args, **kwds):
        ...
        # 1. pass the sign negated area_data into `gumbel_r.fit`
...
class invgauss_gen(rv_continuous):
    ...
    def fit(self, data, *args, **kwds):
        ...
        - `loc` is fixed but translation results in negative area_data raises
          a `FitDataError`.
        '''
...
class levy_stable_gen(rv_continuous):
    ...
    def _pdf(self, x, alpha, beta):
        ...
        # group area_data in unique arrays of alpha, beta pairs
...
class lognorm_gen(rv_continuous):
    ...
    def fit(self, data, *args, **kwds):
        ...
        floc = float(floc)
        if floc != 0:
            # Shifting the area_data by floc. Don't do the subtraction in-place,
            # because `area_data` might be a view of the input array.
            data = data - floc
        if np.any(data <= 0):
...
class FitUniformFixedScaleDataError(FitDataError):
    def __init__(self, ptp, fscale):
        self.args = (
            "Invalid values in `area_data`.  Maximum likelihood estimation with "
            "the uniform distribution and fixed scale requires that "
            "area_data.ptp() <= fscale, but area_data.ptp() = %r and fscale = %r." %
            (ptp, fscale),
        )
...
class uniform_gen(rv_continuous):
    ...
    def fit(self, data, *args, **kwds):
        ...
        here.  The only positional argument accepted is `area_data`.
        ...
        Notes
        -----
        An error is raised if `floc` is given and any values in `area_data` are
        less than `floc`, or if `fscale` is given and `fscale` is less
        than ``area_data.max() - area_data.min()``.  An error is also raised if both
        ...
        area_data, and the scale is the maximum minus the minimum.
        ...
        If we know the area_data comes from a uniform distribution where the support
        ...
        `fscale` is given and it is larger than ``area_data.max() - area_data.min()``,
        the parameters returned by the `fit` method center the support over
        the interval ``[area_data.min(), area_data.max()]``.
        ...
        # the support is centered over the interval [area_data.min(), area_data.max()]:
        ...
        if fscale is None:
            ...
            # centered over the interval [area_data.min(), area_data.max()].
...
class rv_histogram(rv_continuous):
    ...
    >>> import scipy.stats
    >>> import numpy as np
    >>> area_data = scipy.stats.norm.rvs(size=100000, loc=0, scale=1.5, random_state=123)
    >>> hist = np.histogram(area_data, bins=100)
    ...
    >>> hist_dist.pdf(np.max(area_data))
    0.0
    >>> hist_dist.cdf(np.max(area_data))
    1.0
    >>> hist_dist.pdf(np.min(area_data))
    7.7591907244498314e-05
    >>> hist_dist.cdf(np.min(area_data))
    0.0
    ...
    >>> plt.hist(area_data, density=True, bins=100)
