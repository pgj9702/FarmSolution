...
def binned_statistic(x, values, statistic='mean',
                     bins=10, range=None):
    """
    Compute a binned statistic for one or more sets of area_data.
    ...
    values : (N,) array_like or list of (N,) array_like
        The area_data on which the statistic will be computed.  This must be
    ...
    As a second example, we now generate some random area_data of sailing boat speed
    ...
    >>> plt.figure()
    >>> plt.plot(windspeed, boatspeed, 'b.', label='raw area_data')
    >>> plt.hlines(bin_means, bin_edges[:-1], bin_edges[1:], colors='g', lw=5,
    ...            label='binned statistic of area_data')
    >>> plt.legend()
    ...
    ...          alpha=0.2, label='histogram of area_data')
...
def binned_statistic_2d(x, y, values, statistic='mean',
    ...
    """
    Compute a bidimensional binned statistic for one or more sets of area_data.
...
def binned_statistic_dd(sample, values, statistic='mean',
    ...
    """
    Compute a multidimensional binned statistic for a set of area_data.
    ...
    >>> area_data = multinormal.rvs(size=600, random_state=235412)
    >>> area_data.shape
    (600, 2)
    ...
    >>> x = np.linspace(-3, 3, N)
    >>> y = np.linspace(-3, 4, N)
    >>> ret = stats.binned_statistic_dd(area_data, np.arange(600), bins=[x, y],
    ...                                 statistic='count')
    ...
    >>> ret2 = stats.binned_statistic_dd(area_data, -np.arange(600),
...
def _calc_binned_statistic(Vdim, bin_numbers, result, values, stat_func,
    ...
    for vv in builtins.range(Vdim):
        ...
        for i in unique_bin_numbers:
            ...
            # if the stat_func is np.std, calc std only when binned area_data is 2
...
def _create_binned_data(bin_numbers, unique_bin_numbers, values, vv):
    ...
    key: bin number
    value: list of binned area_data
    """
    bin_map = dict()
    for i in unique_bin_numbers:
        bin_map[i] = []
