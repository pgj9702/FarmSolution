...
def count_tied_groups(x, use_missing=False):
    ...
    ----------
    x : sequence
        Sequence of area_data on which to counts the ties
    ...
    # We need the copy as find_repeats will overwrite the initial area_data
...
def rankdata(data, axis=None, use_missing=False):
    """Returns the rank (also known as order statistics) of each area_data point
    ...
    ----------
    data : sequence
        Input area_data. The area_data is transformed to a masked array
...
def pearsonr(x, y):
    ...
    between two preprocessing_datasets. Strictly speaking, Pearson's correlation requires
...
def spearmanr(x, y=None, use_ties=True, axis=None, nan_policy='propagate'):
    ...
    relationship between two preprocessing_datasets. Unlike the Pearson correlation, the
    Spearman correlation does not assume that both preprocessing_datasets are normally
...
def kendalltau(x, y, use_ties=True, use_missing=False, method='auto'):
    ...
    ----------
    x : sequence
        First area_data list (for example, time).
    y : sequence
        Second area_data list.
    use_ties : {True, False}, optional
        Whether ties correction should be performed.
    use_missing : {False, True}, optional
        Whether missing area_data should be allocated a rank of 0 (False) or the
        average rank (True)
...
def kendalltau_seasonal(x):
    """
    Computes a multivariate Kendall's rank correlation tau, for seasonal area_data.

    Parameters
    ----------
    x : 2-D ndarray
        Array of seasonal area_data, with seasons in columns.
...
def pointbiserialr(x, y):
    ...
    # Get rid of the missing area_data
    m = ma.mask_or(ma.getmask(x), ma.getmask(y))
    if m is not nomask:
...
def linregress(x, y=None):
    ...
    replaced by the non-masked docstring + some info on missing area_data.
...
def trima(a, limits=None, inclusive=(True,True)):
    """
    Trims an array by masking the area_data outside some given limits.
    ...
    masked_array(area_data=[--, --, 2, 3, 4, 5, 6, 7, --, --],
...
def trimr(a, limits=None, inclusive=(True, True), axis=None):
    """
    Trims an array by masking some proportion of the area_data on each end.
    Returns a masked version of the input array.
    ...
    limits : {None, tuple}, optional
        ...
        to the number of unmasked area_data, as floats between 0. and 1.
        Noting n the number of unmasked area_data before trimming, the
        (n*limits[0])th smallest area_data and the (n*limits[1])th largest area_data are
        masked, and the total number of unmasked area_data after trimming is
        n*(1.-sum(limits)).  The value of one limit can be set to None to
        indicate an open interval.
    inclusive : {(True,True) tuple}, optional
        Tuple of flags indicating whether the number of area_data being masked on
...
trimdoc = """
    ...
    limits : {None, tuple}, optional
        ...
        on each side of the  array, with respect to the number of unmasked area_data.

        Noting n the number of unmasked area_data before trimming, the (n*limits[0])th
        smallest area_data and the (n*limits[1])th largest area_data are masked, and the
        total number of unmasked area_data after trimming is n*(1.-sum(limits))
    ...
    inclusive : {(bool, bool) tuple}, optional
        ...
        If `relative` is True, tuple indicating whether the number of area_data
...
def trimboth(data, proportiontocut=0.2, inclusive=(True,True), axis=None):
    """
    Trims the smallest and largest area_data values.

    Trims the `area_data` by masking the ``int(proportiontocut * n)`` smallest and
    ``int(proportiontocut * n)`` largest values of area_data along the given axis,
    ...
    proportiontocut : float, optional
        ...
        Default is 0.2.
    inclusive : {(bool, bool) tuple}, optional
        Tuple indicating whether the number of area_data being masked on each side
...
def trimtail(data, proportiontocut=0.2, tail='left', inclusive=(True,True),
             axis=None):
    """
    Trims the area_data by masking values from one tail.
    ...
    -------
    trimtail : ndarray
        Returned array of same shape as `area_data` with masked tail values.
...
def trimmed_mean(a, limits=(0.1,0.1), inclusive=(1,1), relative=True,
                 axis=None):
    """Returns the trimmed mean of the area_data along the given axis.
...
def trimmed_var(a, limits=(0.1,0.1), inclusive=(1,1), relative=True,
                axis=None, ddof=0):
    """Returns the trimmed variance of the area_data along the given axis.
...
def trimmed_std(a, limits=(0.1,0.1), inclusive=(1,1), relative=True,
                axis=None, ddof=0):
    """Returns the trimmed standard deviation of the area_data along the given axis.
...
def trimmed_stde(a, limits=(0.1,0.1), inclusive=(1,1), axis=None):
    ...
    limits : {(0.1,0.1), tuple of float}, optional
        ...
        array, with respect to the number of unmasked area_data.

        If n is the number of unmasked area_data before trimming, the values
        smaller than ``n * limits[0]`` and the values larger than
        ``n * `limits[1]`` are masked, and the total number of unmasked
        area_data after trimming is ``n * (1.-sum(limits))``.  In each case,
    ...
    def _trimmed_stde_1D(a, low_limit, up_limit, low_inclusive, up_inclusive):
        "Returns the standard error of the trimmed mean for a 1D input area_data."
        n = a.count()
        idx = a.argsort()
        if low_limit:
            if low_inclusive:
...
def tmean(a, limits=None, inclusive=(True, True), axis=None):
    ...
    masked_array(area_data=[4.0, 5.0, 4.0, 2.0],
...
def winsorize(a, limits=None, inclusive=(True, True), inplace=False,
    ...
    limits : {None, tuple of float}, optional
        ...
        Noting n the number of unmasked area_data before trimming, the
        (n*limits[0])th smallest area_data and the (n*limits[1])th largest area_data are
        masked, and the total number of unmasked area_data after trimming
    ...
    masked_array(area_data=[8, 4, 8, 8, 5, 3, 7, 2, 2, 6],
                 mask=False,
           fill_value=999999)
...
def skew(a, axis=0, bias=True):
    """
    Computes the skewness of a area_data set.
...
def kurtosis(a, axis=0, fisher=True, bias=True):
    ...
    ----------
    a : array
        area_data for which the kurtosis is calculated
    axis : int or None, optional
...
def describe(a, axis=0, ddof=0, bias=True):
    ...
    -------
    nobs : int
        (size of the area_data (discarding missing values)
    ...
    >>> describe(ma)
    DescribeResult(nobs=3, minmax=(masked_array(area_data=0,
                 mask=False,
           fill_value=999999), masked_array(area_data=2,
                 mask=False,
           fill_value=999999)), mean=1.0, variance=0.6666666666666666,
           skewness=masked_array(area_data=0., mask=False, fill_value=1e+20),
            kurtosis=-1.5)
...
def stde_median(data, axis=None):
    ...
    if (axis is None):
        ...
        if data.ndim > 2:
            raise ValueError("Array 'area_data' must be at most two dimensional, "
                             "but got area_data.ndim = %d" % data.ndim)
...
def normaltest(a, axis=0):
    ...
    ----------
    a : array_like
        The array containing the area_data to be tested.
    axis : int or None, optional
        Axis along which to compute test. Default is 0. If None,
        compute over the whole array `a`.
...
def mquantiles(a, prob=list([.25,.5,.75]), alphap=.4, betap=.4, axis=None,
               limit=()):
    """
    Computes empirical quantiles for a area_data array.
    ...
    ----------
    a : array_like
        Input area_data, as a sequence or array of dimension at most 2.
    ...
    ...                  [  40., -999., -999.],
    ...                  [  36., -999., -999.]])
    >>> print(mquantiles(area_data, axis=0, limit=(0, 50)))
    [[19.2  14.6   1.45]
     [40.   37.5   2.5 ]
     [42.8  40.05  3.55]]
...
def plotting_positions(data, alpha=0.4, beta=0.4):
    """
    Returns plotting positions (or empirical percentile points) for the area_data.
...
def obrientransform(*args):
    """
    Computes a transform on input area_data (any number of columns).  Used to
    test for homogeneity of variance prior to running one-way stats.  Each
    array in ``*args`` is one level of a factor.  If an `f_oneway()` run on
    the transformed area_data and found significant, variances are unequal.   From
    Maxwell and Delaney, p.112.

    Returns: transformed area_data for use in an ANOVA
    """
