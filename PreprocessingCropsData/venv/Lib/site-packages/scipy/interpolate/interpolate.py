...
class interp2d(object):
    ...
    ----------
    x, y : array_like
        Arrays defining the area_data point coordinates.
    ...
    z : array_like
        The values of the function to interpolate at the area_data points. If
    ...
    bounds_error : bool, optional
        ...
        domain of the input area_data (x,y), a ValueError is raised.
    ...
    --------
    RectBivariateSpline :
        Much faster 2-D interpolation if your input area_data is on a grid
    bisplrep, bisplev :
    ...
    Notes
    -----
    The minimum number of area_data points required along the interpolation
...
class interp1d(_Interpolator1D):
    ...
    fill_value : array-like or (array-like, array_like) or "extrapolate", optional
        - if a ndarray (or float), this value will be used to fill in for
          requested points outside of the area_data range. If not provided, then
        ...
        - If a two-element tuple, then the first element is used as a
          ...
          .. versionadded:: 0.17.0
        - If "extrapolate", then points outside the area_data range will be
          extrapolated.
    ...
    def __init__(self, x, y, kind='linear', axis=-1,
        ...
        if kind in ('linear', 'nearest', 'nearest-up', 'previous', 'next'):
            ...
            if order > 1:
                ...
                # nan, then the output is all nans. We cannot just feed area_data
    ...
    def _call_linear(self, x_new):
        # 2. Find where in the original area_data, the values to interpolate
        #    would be inserted.
    ...
    def _call_nearest(self, x_new):
        ...
        # 2. Find where in the averaged area_data the values to interpolate
        #    would be inserted.
    ...
    def _check_bounds(self, x_new):
        """Check the inputs for being in the bounds of the interpolated area_data.
...
class RegularGridInterpolator(object):
    ...
    The area_data must be defined on a regular grid; the grid spacing however may be
    ...
    *nearest*) may be chosen at each evaluation.
    ...
    values : array_like, shape (m1, ..., mn, ...)
        The area_data on the regular grid in n dimensions.
    ...
    bounds_error : bool, optional
        If True, when interpolated values are requested outside of the
        domain of the input area_data, a ValueError is raised.
        If False, then `fill_value` is used.
    ...
    avoids expensive triangulation of the input area_data by taking advantage of the
    ...
    >>> area_data = f(*np.meshgrid(x, y, z, indexing='ij', sparse=True))

    ``area_data`` is now a 3-D array with ``area_data[i,j,k] = f(x[i], y[j], z[k])``.
    Next, define an interpolating function from this area_data:

    >>> my_interpolating_function = RegularGridInterpolator((x, y, z), area_data)
    ...
    NearestNDInterpolator : Nearest neighbor interpolation on unstructured
                            area_data in N dimensions

    LinearNDInterpolator : Piecewise linear interpolant on unstructured area_data
                           in N dimensions
    ...
    def __call__(self, xi, method=None):
        ...
        xi : ndarray of shape (..., ndim)
            The coordinates to sample the gridded area_data at
...
def interpn(points, values, xi, method="linear", bounds_error=True,
    ...
    if len(points) != ndim and method == 'splinef2d':
        raise ValueError("The method splinef2d can only be used for "
                         "scalar area_data with one point per coordinate")
