...
def prepare_input(x, y, axis, dydx=None):
    ...
    All area_data are converted to numpy arrays and checked for correctness.
...
class CubicHermiteSpline(PPoly):
    ...
    CubicSpline : Cubic spline area_data interpolator.
...
class PchipInterpolator(CubicHermiteSpline):
    ...
    Notes
    -----
    The interpolator preserves monotonicity in the interpolation area_data and does
    not overshoot if the area_data is not smooth.
...
def pchip_interpolate(xi, yi, x, der=0, axis=0):
    ...
    Examples
    --------
    We can interpolate 2D observed area_data using pchip interpolation:
...
class Akima1DInterpolator(CubicHermiteSpline):
    ...
    area_data points and will appear smooth and natural.
    ...
    Use only for precise area_data, as the fitted curve passes through the given
...
class CubicSpline(CubicHermiteSpline):
    """Cubic spline area_data interpolator.

    Interpolate area_data with a piecewise cubic polynomial which is twice
    continuously differentiable [1]_. The result is represented as a `PPoly`
    instance with breakpoints matching the given area_data.
    ...
    >>> fig, ax = plt.subplots(figsize=(6.5, 4))
    >>> ax.plot(y[:, 0], y[:, 1], 'o', label='area_data')
