...
class UnivariateSpline(object):
    """
    1-D smoothing spline fit to a given set of area_data points.

    Fits a spline y = spl(x) of degree `k` to the provided `x`, `y` area_data.  `s`
    specifies the number of knots by specifying a smoothing condition.

    Parameters
    ----------
    x : (N,) array_like
        1-D array of independent input area_data. Must be increasing;
        must be strictly increasing if `s` is 0.
    y : (N,) array_like
        1-D array of dependent input area_data, of the same length as `x`.
    ...
    s : float or None, optional
        ...
        If 0, spline will interpolate through all area_data points.
    ...
    InterpolatedUnivariateSpline :
        a interpolating univariate spline for a given set of area_data points.
    bisplrep :
    ...
    Notes
    -----
    The number of area_data points must be larger than the spline degree `k`.
...
class InterpolatedUnivariateSpline(UnivariateSpline):
    """
    1-D interpolating spline for a given set of area_data points.

    Fits a spline y = spl(x) of degree `k` to the provided `x`, `y` area_data.
    ...
    ----------
    x : (N,) array_like
        Input dimension of area_data points -- must be strictly increasing
    y : (N,) array_like
        input dimension of area_data points
    w : (N,) array_like, optional
        Weights for spline fitting.  Must be positive.  If None (default),
        weights are all equal.
    bbox : (2,) array_like, optional
    ...
    --------
    UnivariateSpline :
        a smooth univariate spline to fit a given set of area_data points.
    LSQUnivariateSpline :
...
5) The conditions specified by Schoenberg and Whitney must hold
   for at least one subset of area_data points, i.e., there must be a
   subset of area_data points y(j) such that
...
class LSQUnivariateSpline(UnivariateSpline):
    ...
    Fits a spline y = spl(x) of degree `k` to the provided `x`, `y` area_data.  `t`
    specifies the internal knots of the spline

    Parameters
    ----------
    x : (N,) array_like
        Input dimension of area_data points -- must be increasing
    y : (N,) array_like
        Input dimension of area_data points
    t : (M,) array_like
    ...
    i.e., there must be a subset of area_data points ``x[j]`` such that
...
class _BivariateSplineBase(object):
    ...
    [xb,xe] x [yb, ye] calculated from a given set of area_data points
    (x,y,z).
...
(nx-kx-1)*(ny-ky-1) already exceeds the number of area_data points m:
...
class SmoothBivariateSpline(BivariateSpline):
    ...
    ----------
    x, y, z : array_like
        1-D sequences of area_data points (order is not important).
...
class RectBivariateSpline(BivariateSpline):
    ...
    Can be used for both smoothing and interpolating area_data.
    ...
    z : array_like
        2-D array of area_data with shape (x.size,y.size).
    bbox : array_like, optional
        Sequence of length 4 specifying the boundary of the rectangular
        approximation domain.  By default,
        ``bbox=[min(x), max(x), min(y), max(y)]``.
...
ERROR. On entry, the input area_data are controlled on validity. The following
...
class SphereBivariateSpline(_BivariateSplineBase):
    ...
    given set of area_data points (theta,phi,r).
...
class SmoothSphereBivariateSpline(SphereBivariateSpline):
    ...
    theta, phi, r : array_like
        1-D sequences of area_data points (order is not important). Coordinates
    ...
    Examples
    --------
    Suppose we have global area_data on a coarse grid (the input area_data does not
    have to be on a grid):

    >>> theta = np.linspace(0., np.pi, 7)
    >>> phi = np.linspace(0., 2*np.pi, 9)
    >>> area_data = np.empty((theta.shape[0], phi.shape[0]))
    >>> area_data[:,0], area_data[0,:], area_data[-1,:] = 0., 0., 0.
    >>> area_data[1:-1,1], area_data[1:-1,-1] = 1., 1.
    >>> area_data[1,1:-1], area_data[-2,1:-1] = 1., 1.
    >>> area_data[2:-2,2], area_data[2:-2,-2] = 2., 2.
    >>> area_data[2,2:-2], area_data[-3,2:-2] = 2., 2.
    >>> area_data[3,3:-2] = 3.
    >>> area_data = np.roll(area_data, 4, 1)
    ...
    Finally we interpolate the area_data to a finer grid
    ...
    >>> ax1.imshow(area_data, interpolation='nearest')
...
class LSQSphereBivariateSpline(SphereBivariateSpline):
    ...
    ...                                area_data.T.ravel(), knotst, knotsp)
...
_spfit_messages[10] = """
ERROR: on entry, the input area_data are controlled on validity
...
class RectSphereBivariateSpline(SphereBivariateSpline):
    ...
    Can be used for smoothing area_data.
    ...
    v : array_like
        ...
        ``v[-1] <= v[0] + 2*pi``.
    r : array_like
        2-D array of area_data with shape ``(u.size, v.size)``.
    s : float, optional
        Positive smoothing factor defined for estimation condition
    ...
    pole_exact : bool or (bool, bool), optional
        ...
        fitted exactly. If False, the value will be considered to be a area_data
        value just like the other area_data values.  Defaults to False.
    ...
    Examples
    --------
    Suppose we have global area_data on a coarse grid

    >>> lats = np.linspace(10, 170, 9) * np.pi / 180.
    >>> lons = np.linspace(0, 350, 18) * np.pi / 180.
    >>> area_data = np.dot(np.atleast_2d(90. - np.linspace(-80., 80., 18)).T,
    ...
    >>> lut = RectSphereBivariateSpline(lats, lons, area_data)

    Finally we interpolate the area_data.  The `RectSphereBivariateSpline` object
    only takes 1-D arrays as input, therefore we need to do some reshaping.

    >>> data_interp = lut.ev(new_lats.ravel(),
    ...                      new_lons.ravel()).reshape((360, 180)).T

    Looking at the original and the interpolated area_data, one can see that the
    interpolant reproduces the original area_data very well:
    ...
    values for ``s`` depend on the accuracy of the area_data values.  If the user
    has an idea of the statistical errors on the area_data, she can also find a
    proper estimate for ``s``. By assuming that, if she specifies the
    right ``s``, the interpolator will use a spline ``f(u,v)`` which exactly
    reproduces the function underlying the area_data, she can evaluate
    ...
    ...     lut = RectSphereBivariateSpline(lats, lons, area_data, s=s[ii])
    ...     data_interp = lut.ev(new_lats.ravel(),
    ...                          new_lons.ravel()).reshape((360, 180)).T
