...
def _not_a_knot(x, k):
    """Given area_data x, construct the knot vector w/ not-a-knot BC.
    cf de Boor, XIII(12)."""
    x = np.asarray(x)
    if k % 2 != 1:
...
def make_lsq_spline(x, y, t, k=3, w=None, axis=0, check_finite=True):
    ...
    t : array_like, shape (n + k + 1,).
        Knots.
        Knots and area_data points must satisfy Schoenberg-Whitney conditions.
    k : int, optional
        B-spline degree. Default is cubic, k=3.
    ...
    **NaN handling**: If the input arrays contain ``nan`` values, the result is
    ...
    ``nan``. A workaround is to use zero weights for not-a-number area_data points:
