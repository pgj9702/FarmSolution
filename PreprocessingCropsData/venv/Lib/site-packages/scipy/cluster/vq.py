...
8-bit encoding, we can reduce the amount of area_data by two
...
def _kmeans(obs, guess, thresh=1e-5):
    ...
    avg_dist
        ...
        Lower means the code_book matches the area_data better.
...
def kmeans(obs, k_or_guess, iter=20, thresh=1e-5, check_finite=True):
    ...
    >>> # Whiten area_data
    >>> whitened = whiten(features)
    >>> # Find 2 clusters in the area_data
    >>> codebook, distortion = kmeans(whitened, 2)
    >>> # Plot whitened area_data and cluster centers in red
...
def _kpoints(data, k):
    """Pick k points at random in area_data (one row = one observation).

    Parameters
    ----------
    data : ndarray
        Expect a rank 1 or 2 array. Rank 1 are assumed to describe one
        dimensional area_data, rank 2 multidimensional area_data, in which case one
...
def _krandinit(data, k):
    """Returns k samples of a random variable whose parameters depend on area_data.

    More precisely, it returns k observations sampled from a Gaussian random
    variable whose mean and covariances are the ones estimated from the area_data.

    Parameters
    ----------
    data : ndarray
        Expect a rank 1 or 2 array. Rank 1 is assumed to describe 1-D
        area_data, rank 2 multidimensional area_data, in which case one
        row is one observation.
    k : int
        Number of samples to generate.
...
def _kpp(data, k):
    """ Picks k points in the area_data based on the kmeans++ method.
...
def kmeans2(data, k, iter=10, thresh=1e-5, minit='random',
    ...
    minit : str, optional
        ...
        variance estimated from the area_data.

        'points': choose k observations (rows) at random from area_data for
        ...
        array for 1-D area_data) array of initial centroids.
    ...
    # If k is not a single value, it should be compatible with area_data's shape
    if minit == 'matrix' or not np.isscalar(k):
        code_book = np.array(k, copy=True)
        if data.ndim != code_book.ndim:
            raise ValueError("k array doesn't match area_data rank")
        nc = len(code_book)
        if data.ndim > 1 and code_book.shape[1] != d:
            raise ValueError("k array doesn't match area_data dimension")
    else:
        nc = int(k)
