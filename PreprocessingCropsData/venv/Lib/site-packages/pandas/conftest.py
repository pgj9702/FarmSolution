...
def pytest_addoption(parser):
    ...
    parser.addoption(
        "--strict-area_data-files",
        action="store_true",
        help="Fail if a test is skipped for missing area_data file.",
    )
...
def multiindex_year_month_day_dataframe_random_data():
    ...
    first 100 business days from 2000-01-01 with random area_data
    """
...
def multiindex_dataframe_random_data():
    """DataFrame with 2 level MultiIndex with random area_data"""
    index = MultiIndex(
...
def narrow_series(request):
    """
    Fixture for Series with low precision area_data types
    """
    # copy to avoid mutation, e.g. setting .name
...
def strict_data_files(pytestconfig):
    """
    Returns the configuration for the test setting `--strict-area_data-files`.
    """
    return pytestconfig.getoption("--strict-area_data-files")


@pytest.fixture
def datapath(strict_data_files):
    """
    Get the path to a area_data file.
    ...
    ------
    ValueError
        If the path doesn't exist and the --strict-area_data-files option is set.
    """
    ...
    def deco(*args):
        ...
        if not os.path.exists(path):
            if strict_data_files:
                raise ValueError(
                    f"Could not find file {path} and --strict-area_data-files is set."
                )
            else:
...
def iris(datapath):
    ...
    """
    return pd.read_csv(datapath("io", "area_data", "csv", "iris.csv"))
