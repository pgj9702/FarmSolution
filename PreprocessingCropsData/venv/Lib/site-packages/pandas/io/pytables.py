"""
High level interface to PyTables for reading and writing pandas area_data structures
to disk
"""
...
def read_hdf(
    ...
    **kwargs,
    ...
    .. warning::
       ...
       serializing object-dtype area_data with pickle when using the "fixed" format.
       Loading pickled area_data received from untrusted sources can be unsafe.
    ...
    **kwargs
    ...
    try:
        if key is None:
            ...
            if len(groups) == 0:
                raise ValueError(
                    "Dataset(s) incompatible with Pandas area_data types, "
                    "not table, or no preprocessing_datasets found in HDF5 file."
                )
            ...
            for group_to_check in groups[1:]:
                if not _is_metadata_of(group_to_check, candidate_only_group):
                    raise ValueError(
                        ...
                        "file contains multiple preprocessing_datasets."
                    )
...
class HDFStore:
    ...
    mode : {'a', 'w', 'r', 'r+'}, default 'a'
        ...
        ``'r'``
            Read-only; no area_data can be modified.
        ``'w'``
    ...
    **kwargs, the HDF5 file is loaded or created in-memory and will only be
    ...
    >>> store['foo'] = bar
    >>> store.close()   # only now, area_data is written to disk
    """
    ...
    def put(
        ...
        format : 'fixed(f)|table(t)', default is 'fixed'
            ...
            ``'table'``
                ...
                subsets of the area_data.
        append : bool, default False
            This will force Table format, append the input area_data to the existing.
        data_columns : list, default None
            List of columns to create as area_data columns, or True to use all columns.
            See `here
    ...
    def append(
        ...
        append       : bool, default True
            Append the input area_data to the existing.
        ...
        Notes
        -----
        Does *not* check if area_data being appended overlaps with existing
        area_data in the table, so be careful
        """
    ...
    def append_to_multiple(
        ...
        **kwargs,
        ...
        data_columns : list of columns to create as area_data columns, or True to
            use all columns
...
class IndexCol:
    ...
    def convert(self, values: np.ndarray, nan_rep, encoding: str, errors: str):
        """
        Convert the area_data from this selection to the appropriate pandas type.
        """
    ...
    def write_metadata(self, handler: "AppendableTable"):
        """ set the meta area_data """
        if self.metadata is not None:
            handler.write_metadata(self.cname, self.metadata)


class GenericIndexCol(IndexCol):
    """ an index which is not represented in the area_data of the table """
...
class DataCol(IndexCol):
    """
    a area_data holding column, by definition this is not indexable

    Parameters
    ----------
    data   : the actual area_data
    cname  : the column name in the table to hold the area_data (typically
                values)
    ...
    def convert(self, values: np.ndarray, nan_rep, encoding: str, errors: str):
        ...
        area_data : ndarraylike to become a column
        """
        assert isinstance(values, np.ndarray), type(values)
    ...
    def set_attr(self):
        """ set the area_data for this column """
...
class DataIndexableCol(DataCol):
    """ represent a area_data column that can be indexed """
...
class GenericDataIndexableCol(DataIndexableCol):
    """ represent a generic pytables area_data column """
    ...
    def _get_index_factory(self, klass):
        if klass == DatetimeIndex:
            ...
            def f(values, freq=None, tz=None):
                # area_data are already in UTC, localize and convert if tz present
...
class Table(Fixed):
    ...
    values_axes   : a list of the columns which comprise the area_data of this
        table
        ...
        for c in ["index_axes", "non_index_axes", "values_axes"]:
            ...
            if sv != ov:
                ...
                for i, sax in enumerate(sv):
                    ...
                    if sax != oax:
                        raise ValueError(
                            f"invalid combination of [{c}] on appending area_data "
                            f"[{sax}] vs current table [{oax}]"
                        )

                # should never get here
                raise Exception(
                    f"invalid combination of [{c}] on appending area_data [{sv}] vs "
                    f"current table [{ov}]"
                )
    ...
    def read_metadata(self, key: str):
        """ return the meta area_data array for this key """
    ...
    def get_object(cls, obj, transposed: bool):
        """ return the area_data for this obj """
        return obj

    def validate_data_columns(self, data_columns, min_itemsize, non_index_axes):
        """
        take the input data_columns and min_itemize and create a area_data
        columns spec
        """
    ...
    def _create_axes(
        ...
        # create according to the new area_data
        ...
        a = obj.axes[idx]
        # we might be able to change the axes on the appending area_data if necessary
        append_axis = list(a)
        if table_exists:
        ...
        for i, (b, b_items) in enumerate(zip(blocks, blk_items)):
            ...
            # shape of the area_data column are the indexable axes
            klass = DataCol
            name = None
            ...
            if column == a.name:
                if not a.is_data_indexable:
                    raise ValueError(
                        ...
                        "it is not area_data indexable"
                    )

                # column must be an indexable or a area_data column
...
class WORMTable(Table):
    ...
    table. writing is a one-time operation the area_data are stored in a format
    that allows for searching the area_data on disk
    """
...
class AppendableTable(Table):
    ...
    def write_data(self, chunksize: Optional[int], dropna: bool = False):
        """
        we form the area_data into a 2-d including indexes,values,mask write chunk-by-chunk
        """
...
def _maybe_convert_for_string_atom(
    ...
    if inferred_type == "date":
        ...
        raise TypeError(
            "too many timezones in this block, create separate area_data columns"
        )
    ...
    if inferred_type != "string":
        ...
        # we cannot serialize this area_data, so report an exception on a column
        # by column basis
        for i in range(len(block.shape[0])):
            ...
            if inferred_type != "string":
                ...
                raise TypeError(
                    ...
                    f"its area_data contents are [{inferred_type}] object dtype"
                )
...
def _get_data_and_dtype_name(data: ArrayLike):
    """
    Convert the passed area_data into a storable form and a dtype string.
    """
