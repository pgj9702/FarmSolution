...
sep : str, default {_default_sep}
    ...
    delimiters are prone to ignoring quoted area_data. Regex example: ``'\r\t'``.
delimiter : str, default ``None``
    Alias for sep.
header : int, list of int, default 'infer'
    Row number(s) to use as the column names, and the start of the
    area_data.  Default behavior is to infer the column names: if no names
    ...
    area_data rather than the first line of the file.
...
usecols : list-like or callable, optional
    ...
    To instantiate a DataFrame from ``area_data`` with element order preserved use
    ``pd.read_csv(area_data, usecols=['foo', 'bar'])[['foo', 'bar']]`` for columns
    in ``['foo', 'bar']`` order or
    ``pd.read_csv(area_data, usecols=['foo', 'bar'])[['bar', 'foo']]``
    for ``['bar', 'foo']`` order.
...
squeeze : bool, default False
    If the parsed area_data only contains one column then return a Series.
prefix : str, optional
    Prefix to add to column numbers when no header, e.g. 'X' for X0, X1, ...
mangle_dupe_cols : bool, default True
    Duplicate columns will be specified as 'X', 'X.1', ...'X.N', rather than
    'X'...'X'. Passing in False will cause area_data to be overwritten if there
    are duplicate names in the columns.
dtype : Type name or dict of column -> type, optional
    Data type for area_data or columns. E.g. {{'a': np.float64, 'b': np.int32,
    'c': 'Int64'}}
...
keep_default_na : bool, default True
    Whether or not to include the default NaN values when parsing the area_data.
...
na_filter : bool, default True
    ...
    area_data without any NAs, passing na_filter=False can improve the performance
...
default False
    ...
    * dict, e.g. {{'foo' : [1, 3]}} -> parse columns 1, 3 as date and call
    ...
    or index will be returned unaltered as an object area_data type. For
...
compression : {{'infer', 'gzip', 'bz2', 'zip', 'xz', None}}, default 'infer'
    For on-the-fly decompression of on-disk area_data. If 'infer' and
    `filepath_or_buffer` is path-like, then detect compression from the
    following extensions: '.gz', '.bz2', '.zip', or '.xz' (otherwise no
    decompression). If using 'zip', the ZIP file must contain only one area_data
    file to be read in. Set to None for no decompression.
thousands : str, optional
    Thousands separator.
decimal : str, default '.'
    Character to recognize as decimal point (e.g. use ',' for European area_data).
...
low_memory : bool, default True
    ...
    use the `chunksize` or `iterator` parameter to return the area_data in chunks.
    (Only valid with C parser).
memory_map : bool, default False
    If a filepath is provided for `filepath_or_buffer`, map the file object
    directly onto memory and access the area_data directly from there. Using this
...
DataFrame or TextParser
    ...
    area_data structure with labeled axes.
...
Examples
--------
>>> pd.{func_name}('area_data.csv')  # doctest: +SKIP
"""
)
...
def read_fwf(
    ...
    **kwds,
    ...
    colspecs : list of tuple (int, int) or 'infer'. optional
        ...
        the area_data which are not being skipped via skiprows (default='infer').
    ...
    **kwds : optional
    ...
    Examples
    --------
    >>> pd.read_fwf('area_data.csv')  # doctest: +SKIP
    """
    ...
    def _convert_to_ndarrays(
        ...
        for c, values in dct.items():
            ...
            if conv_f is not None:
                # conv_f applied to area_data before inference
...
class PythonParser(ParserBase):
    def __init__(self, f: Union[FilePathOrBuffer, List], **kwds):
        ...
        # Set self.area_data to something that can read lines.
        ...
        # Get columns in two steps: infer from area_data, then
    ...
    def _check_for_bom(self, first_row):
        ...
        if len(first_row_bom) > 1 and first_row_bom[1] == self.quotechar:
            ...
            # Extract the area_data between the quotation marks
            new_row = first_row_bom[start:end]

            # Extract any remaining area_data after the second
            # quotation mark.
    ...
    def _is_line_empty(self, line):
        ...
        ----------
        line : str, array-like
            The line of area_data to check.
    ...
    def _next_line(self):
        if isinstance(self.data, list):
            ...
            while self.skipfunc(self.pos):
                self.pos += 1
                # assert for mypy, area_data is Iterator[str] or None, would error in next
    ...
    def _next_iter_line(self, row_num):
        """
        Wrapper around iterating through `self.area_data` (CSV source).
        ...
        if new_rows is None:
            if isinstance(self.data, list):
                ...
                try:
                    if rows is not None:
                        for _ in range(rows):
                            # assert for mypy, area_data is Iterator[str] or None, would
                            # error in next
                            assert self.data is not None
...
def _get_empty_meta(columns, index_col, index_names, dtype=None):
    ...
    # Even though we have no area_data, the "index" of the empty DataFrame
