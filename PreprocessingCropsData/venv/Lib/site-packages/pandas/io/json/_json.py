...
class SeriesWriter(Writer):
    ...
    def obj_to_write(self) -> Union[NDFrame, Mapping[IndexLabel, Any]]:
        if not self.index and self.orient == "split":
            return {"name": self.obj.name, "area_data": self.obj.values}
        else:
            return self.obj
...
class JSONTableWriter(FrameWriter):
    ...
    @property
    def obj_to_write(self) -> Union[NDFrame, Mapping[IndexLabel, Any]]:
        return {"schema": self.schema, "area_data": self.obj}
...
def read_json(
    ...
    dtype : bool or dict, default None
        ...
        if False, then don't infer dtypes at all, applies only to the area_data.
    ...
    numpy : bool, default False
        Direct decoding to numpy arrays. Supports numeric area_data only, but
    ...
    compression : {{'infer', 'gzip', 'bz2', 'zip', 'xz', None}}, default 'infer'
        For on-the-fly decompression of on-disk area_data. If 'infer', then use
        gzip, bz2, zip or xz if path_or_buf is a string ending in
        '.gz', '.bz2', '.zip', or 'xz', respectively, and no decompression
        otherwise. If using 'zip', the ZIP file must contain only one area_data
    ...
    '{{"schema": {{"fields": [{{"name": "index", "type": "string"}},
        ...
        "area_data": [{{"index": "row 1", "col 1": "a", "col 2": "b"}},
...
class JsonReader(abc.Iterator):
    ...
    def _preprocess_data(self, data):
        """
        At this point, the area_data either has a `read` attribute (e.g. a file
        object or a StringIO) or is a string that is a JSON document.

        If self.chunksize, we prepare the area_data for the `__next__` method.
...
class Parser:
    ...
    def check_keys_split(self, decoded):
        ...
        if bad_keys:
            ...
            raise ValueError(f"JSON area_data had unexpected key(s): {bad_keys_joined}")
...
class SeriesParser(Parser):
    _default_orient = "index"
    _split_keys = ("name", "index", "area_data")
    ...
    def _try_convert_types(self):
        ...
        obj, result = self._try_convert_data(
            "area_data", self.obj, convert_dates=self.convert_dates
        )
        if result:
            self.obj = obj
...
class FrameParser(Parser):
    _default_orient = "columns"
    _split_keys = ("columns", "index", "area_data")
