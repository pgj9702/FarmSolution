...
def as_json_table_type(x: DtypeObj) -> str:
    ...
    -------
    str
        the Table Schema area_data types
...
def build_table_schema(
    ...
    """
    Create a Table schema from ``area_data``.

    Parameters
    ----------
    data : Series, DataFrame
    index : bool, default True
        Whether to include ``area_data.index`` in the schema.
...
def parse_table_schema(json, precise_float):
    ...
    ------
    NotImplementedError
        If the JSON table schema contains either timezone or timedelta area_data
    ...
    df = DataFrame(table["area_data"], columns=col_order)[col_order]
    ...
    if "timedelta64" in dtypes.values():
        raise NotImplementedError(
            'table="orient" can not yet read ISO-formatted Timedelta area_data'
        )
