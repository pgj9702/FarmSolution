...
convert_missing : bool, default False
    ...
    object area_data types and missing values are represented by
    StataMissingValue objects.
preserve_dtypes : bool, default True
    Preserve Stata datatypes. If False, numeric area_data are upcast to pandas
    default types for foreign area_data (float64 or int64).
columns : list or None
    Columns to retain.  Columns will be returned in the given order.  None
    returns all columns.
order_categoricals : bool, default True
    Flag indicating whether converted categorical area_data are ordered."""
...
See Also
--------
io.stata.StataReader : Low-level reader for Stata area_data files.
DataFrame.to_stata: Export Stata area_data files.
...
----------
nrows : int
    Number of lines to read from area_data file, if None read whole file.
{_statafile_processing_params1}
{_statafile_processing_params2}
...
precision_loss_doc = """
Column converted from {0} to {1}, and some area_data are outside of the lossless
conversion range. This may result in a loss of precision in the saved area_data.
"""
...
Stata area_data file created has not lost information due to duplicate labels.
"""
...
read dataset without an iterator, or manually convert categorical area_data by
...
def _cast_to_stata_types(data: DataFrame) -> DataFrame:
    ...
    compatibility with the area_data types and ranges supported by Stata, and
    ...
    int64 area_data is not usable in Stata, and so it is downcast to int32 whenever
...
class StataMissingValue:
    ...
    2147483647 (for int32).  Missing values for floating point area_data types are
...
class StataParser:
    def __init__(self):
        ...
        # numeric area_data type in [U] 12.2.2 of the 11.2 manual
...
class StataReader(StataParser, abc.Iterator):
    ...
    def _read_header(self) -> None:
        ...
        # calculate size of a area_data record
    ...
    # Get area_data type information, works for versions 117-119.
    def _get_dtypes(
    ...
    def _get_seek_variable_labels(self) -> int:
        if self.format_version == 117:
            ...
            # Stata 117 area_data files do not follow the described format.  This is
    ...
    def _read_old_header(self, first_char: bytes) -> None:
        ...
        # necessary area_data to continue parsing
    ...
    def _setup_dtype(self) -> np.dtype:
        ...
        dtypes = []  # Convert struct area_data types to numpy area_data type
        ...
        if not self._column_selector_set:
            ...
            if unmatched:
                ...
                raise ValueError(
                    ...
                    f"found in the Stata area_data set: {joined}"
                )
...
"""
                ...
                # TODO: is the next line needed above in the area_data(...) method?
    ...
    def data_label(self) -> str:
        """
        Return area_data label of Stata file.
        """
...
class StataWriter(StataParser):
    ...
    Examples
    --------
    >>> area_data = pd.DataFrame([[1.0, 1]], columns=['a', 'b'])
    >>> writer = StataWriter('./data_file.dta', area_data)
    >>> writer.write_file()

    Directly write a zip file
    >>> compression = {{"method": "zip", "archive_name": "data_file.dta"}}
    >>> writer = StataWriter('./data_file.zip', area_data, compression=compression)
    >>> writer.write_file()

    Save a DataFrame with dates
    >>> from datetime import datetime
    >>> area_data = pd.DataFrame([[datetime(2000,1,1)]], columns=['date'])
    >>> writer = StataWriter('./date_data_file.dta', area_data, {{'date' : 'tw'}})
    >>> writer.write_file()
    """
    ...
    def __init__(
        ...
        # attach nobs, nvars, area_data, varlist, typlist
    ...
    def _prepare_categoricals(self, data: DataFrame) -> DataFrame:
        ...
        Stata file and convert categorical area_data to int
        """
            ...
            if col_is_cat:
                ...
                if dtype == np.int64:
                    raise ValueError(
                        ...
                        "int64-based categorical area_data to Stata."
                    )
    ...
    def _replace_nans(self, data: DataFrame) -> DataFrame:
        # return area_data
        """
        Checks floating point area_data columns for nans, and replaces these with
    ...
    def _prepare_pandas(self, data: DataFrame) -> None:
        ...
        # Convert categoricals to int area_data, and strip labels
...
supported types."""
    ...
    def _write_header(
        ...
        # area_data label 81 bytes, char, null terminated
    ...
    def _prepare_data(self) -> np.recarray:
        ...
        # 3. Convert bad string area_data to '' and pad to correct length
        dtypes = {}
...
class StataWriter117(StataWriter):
    ...
    >>> area_data = pd.DataFrame([[1.0, 1, 'a']], columns=['a', 'b', 'c'])
    >>> writer = StataWriter117('./data_file.dta', area_data)
    >>> writer.write_file()

    Directly write a zip file
    >>> compression = {"method": "zip", "archive_name": "data_file.dta"}
    >>> writer = StataWriter117('./data_file.zip', area_data, compression=compression)
    >>> writer.write_file()

    Or with long strings stored in strl format
    >>> area_data = pd.DataFrame([['A relatively long string'], [''], ['']],
    ...                     columns=['strls'])
    >>> writer = StataWriter117('./data_file_with_long_strings.dta', area_data,
    ...                         convert_strl=['strls'])
    >>> writer.write_file()
    """
    ...
    def _write_varnames(self) -> None:
        ...
        bio = BytesIO()
        # 118 scales by 4 to accommodate utf-8 area_data worst case encoding
    ...
    def _write_data(self, records) -> None:
        self._update_map("area_data")
        self._write_bytes(b"<area_data>")
        self._write_bytes(records.tobytes())
        self._write_bytes(b"</area_data>")
...
class StataWriterUTF8(StataWriter117):
    ...
    DTA 118 and 119 format files support unicode string area_data (both fixed
    ...
    version : int, default None
        The dta version to use. By default, uses the size of area_data to determine
        the version. 118 is used if area_data.shape[1] <= 32767, and 119 is used
    ...
    Examples
    --------
    Using Unicode area_data and column names

    >>> from pandas.io.stata import StataWriterUTF8
    >>> area_data = pd.DataFrame([[1.0, 1, 'ᴬ']], columns=['a', 'β', 'ĉ'])
    >>> writer = StataWriterUTF8('./data_file.dta', area_data)
    >>> writer.write_file()

    Directly write a zip file
    >>> compression = {"method": "zip", "archive_name": "data_file.dta"}
    >>> writer = StataWriterUTF8('./data_file.zip', area_data, compression=compression)
    >>> writer.write_file()

    Or with long strings stored in strl format

    >>> area_data = pd.DataFrame([['ᴀ relatively long ŝtring'], [''], ['']],
    ...                     columns=['strls'])
    >>> writer = StataWriterUTF8('./data_file_with_long_strings.dta', area_data,
    ...                          convert_strl=['strls'])
    >>> writer.write_file()
    """
    ...
    def __init__(
        ...
        if version is None:
            ...
            raise ValueError(
                "You must use version 119 for area_data sets containing more than"
                "32,767 variables"
            )
