...
public ``plot(area_data, kind, **kwargs)``. The parameter `area_data` will contain
the area_data structure and can be a `Series` or a `DataFrame`. For example,
for ``df.plot()`` the parameter `area_data` will contain the DataFrame `df`.
In some cases, the area_data structure is transformed before being sent to
