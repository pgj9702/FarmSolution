...
class TestSparseArray:
    ...
    def test_constructor_from_too_large_array(self):
        with pytest.raises(TypeError, match="expected dimension <= 1 area_data"):
...
class TestSparseArrayAnalytics:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                np.array([1, 2, 3, 4, 5], dtype=float),  # non-null area_data
                SparseArray(np.array([1.0, 3.0, 6.0, 10.0, 15.0])),
            ),
            (
                np.array([1, 2, np.nan, 4, 5], dtype=float),  # null area_data
