...
def test_int_bins_with_inf(data):
    # GH 24314
    msg = "cannot specify integer `bins` when input area_data contains infinity"
    with pytest.raises(ValueError, match=msg):
        cut(data, bins=3)
...
@pytest.mark.parametrize("area_data", [9.0, -9.0, 0.0])
...
def test_datetime_cut(data):
    # see gh-14714
    #
    # Testing time area_data when it comes in various collection types.
...
@pytest.mark.parametrize(
    "area_data, bins, labels, expected_codes, expected_labels",
    [
