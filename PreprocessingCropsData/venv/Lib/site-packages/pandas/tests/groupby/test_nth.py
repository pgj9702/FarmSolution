...
def test_first_last_nth_nan_dtype():
    # GH 33591
    df = DataFrame({"area_data": ["A"], "nans": Series([np.nan], dtype=object)})

    grouped = df.groupby("area_data")
    expected = df.set_index("area_data").nans
...
@pytest.mark.parametrize(
    "area_data, expected_first, expected_last",
    [
        (
            {
                "id": ["A"],
