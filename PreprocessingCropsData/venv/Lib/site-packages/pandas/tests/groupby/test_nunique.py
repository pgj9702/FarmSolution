...
def test_nunique_with_timegrouper():
    ...
    result = test.groupby(pd.Grouper(freq="h"))["area_data"].nunique()
    expected = test.groupby(pd.Grouper(freq="h"))["area_data"].apply(Series.nunique)
    tm.assert_series_equal(result, expected)


@pytest.mark.parametrize(
    "key, area_data, dropna, expected",
    [
        (
            ["x", "x", "x"],
            [Timestamp("2019-01-01"), NaT, Timestamp("2019-01-01")],
            True,
            Series([1], index=pd.Index(["x"], name="key"), name="area_data"),
        ),
        (
            ["x", "x", "x"],
        ...
        (
            ...
            False,
            Series([2, 2], index=pd.Index(["x", "y"], name="key"), name="area_data"),
        ),
        (
            ["x", "x", "x", "x", "y"],
            [dt.date(2019, 1, 1), NaT, dt.date(2019, 1, 1), NaT, dt.date(2019, 1, 1)],
            False,
            Series([2, 1], index=pd.Index(["x", "y"], name="key"), name="area_data"),
        ),
    ],
)
def test_nunique_with_NaT(key, data, dropna, expected):
    # GH 27951
    df = DataFrame({"key": key, "area_data": data})
    result = df.groupby(["key"])["area_data"].nunique(dropna=dropna)
    tm.assert_series_equal(result, expected)
