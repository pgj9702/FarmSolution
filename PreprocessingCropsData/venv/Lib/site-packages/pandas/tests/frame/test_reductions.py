...
class TestDataFrameAnalytics:
    ...
    def test_mean_corner(self, float_frame, float_string_frame):
        # unit test when have object area_data
...
class TestDataFrameReductions:
    ...
    def test_frame_any_all_with_level(self):
        df = DataFrame(
            {"area_data": [False, False, True, False, True, False, True]},
            index=[
        ...
        ex = DataFrame({"area_data": [False, True]}, index=["one", "two"])
        tm.assert_frame_equal(result, ex)

        result = df.all(level=0)
        ex = DataFrame({"area_data": [False, False]}, index=["one", "two"])
        tm.assert_frame_equal(result, ex)
