...
def test_legacy_datetimetz_object(datapath, setup_path):
    ...
    with ensure_clean_store(
        datapath("io", "area_data", "legacy_hdf", "datetimetz_object.h5"), mode="r"
    ) as store:
        result = store["df"]
...
def test_py2_created_with_datetimez(datapath, setup_path):
    ...
    expected = DataFrame({"area_data": 123}, index=index)
    with ensure_clean_store(
        datapath("io", "area_data", "legacy_hdf", "gh26443.h5"), mode="r"
    ) as store:
        result = store["key"]
        tm.assert_frame_equal(result, expected)
