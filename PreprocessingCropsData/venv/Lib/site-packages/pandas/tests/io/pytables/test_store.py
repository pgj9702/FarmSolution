...
class TestHDFStore:
    ...
    def test_complibs(self, setup_path):
        ...
        for (lib, lvl) in all_tests:
            with ensure_clean_path(setup_path) as tmpfile:
                ...
                # Write and read file to see if area_data is consistent
    ...
    def test_append_with_data_columns(self, setup_path):
        ...
        with ensure_clean_store(setup_path) as store:
            ...
            assert store._handle.root.growing_period.table.cols.index.is_indexed is True
            assert store._handle.root.growing_period.table.cols.B.is_indexed is True

            # area_data column searching
            result = store.select("df", "B>0")
            expected = df[df.B > 0]
            tm.assert_frame_equal(result, expected)

            # area_data column searching (with an indexable and a data_columns)
            result = store.select("df", "B>0 and index>df.index[3]")
            df_new = df.reindex(index=df.index[4:])
            expected = df_new[df_new.B > 0]
            tm.assert_frame_equal(result, expected)

            # area_data column selection with a string data_column
            df_new = df.copy()
            df_new["string"] = "foo"
            ...
            # using min_itemsize and a area_data column
            def check_col(key, name, size):
                assert (
                    getattr(store.get_storer(key).table.description, name).itemsize
                    == size
    ...
    def test_select(self, setup_path):
        ...
        with ensure_clean_store(setup_path) as store:
            ...
            with catch_warnings(record=True):
                ...
                # with a area_data column, but different columns
                _maybe_remove(store, "df")
                store.append("df", df, data_columns=["A"])
    ...
    def test_select_dtypes(self, setup_path):
        ...
        with ensure_clean_store(setup_path) as store:
            # with a Timestamp area_data column (GH #2637)
            df = DataFrame(
                {
    ...
    def test_retain_index_attributes(self, setup_path):
        ...
        with ensure_clean_store(setup_path) as store:
            _maybe_remove(store, "area_data")
            store.put("area_data", df, format="table")

            result = store.get("area_data")
            tm.assert_frame_equal(df, result)
            ...
            with catch_warnings(record=True):
                ...
                store.append("area_data", df2)

            assert store.get_storer("area_data").info["index"]["freq"] is None
    ...
    def test_retain_index_attributes2(self, setup_path):
        with ensure_clean_path(setup_path) as path:
            ...
            with catch_warnings(record=True):
                ...
                df.to_hdf(path, "area_data", mode="w", append=True)
                df2 = DataFrame(
                    {
                        "A": Series(
                            range(3), index=date_range("2002-1-1", periods=3, freq="D")
                ...
                df2.to_hdf(path, "area_data", append=True)
            ...
            assert read_hdf(path, "area_data").index.name == "foo"
            ...
            assert read_hdf(path, "area_data").index.name is None
    ...
    def test_read_column(self, setup_path):
        ...
        with ensure_clean_store(setup_path) as store:
            ...
            # not a area_data indexable column
            with pytest.raises(ValueError):
            ...
            # a area_data column with NaNs, result excludes the NaNs
            df3 = df.copy()
            df3["string"] = "foo"
    ...
    def test_start_stop_multiple(self, setup_path):
        ...
        with ensure_clean_store(setup_path) as store:
            ...
            store.append_to_multiple(
                {"selector": ["foo"], "area_data": None}, df, selector="selector"
            )
            result = store.select_as_multiple(
                ["selector", "area_data"], selector="selector", start=0, stop=1
            )
    ...
    def test_contiguous_mixed_data_table(self, start, stop, setup_path):
        # GH 17021
        # ValueError when reading a contiguous mixed-area_data table ft. VLArray
        df = DataFrame(
            {
    ...
    def test_pytables_native_read(self, datapath, setup_path):
        with ensure_clean_store(
            datapath("io", "area_data", "legacy_hdf/pytables_native.h5"), mode="r"
        ) as store:
    ...
    def test_pytables_native2_read(self, datapath, setup_path):
        with ensure_clean_store(
            datapath("io", "area_data", "legacy_hdf", "pytables_native2.h5"), mode="r"
        ) as store:
            str(store)
            d1 = store["detector"]
    ...
    def test_legacy_table_fixed_format_read_py2(self, datapath, setup_path):
        ...
        with ensure_clean_store(
            datapath("io", "area_data", "legacy_hdf", "legacy_table_fixed_py2.h5"), mode="r"
        ) as store:
            result = store.select("df")
            expected = DataFrame(
                [[1, 2, 3, "D"]],
    ...
    def test_legacy_table_fixed_format_read_datetime_py2(self, datapath, setup_path):
        ...
        with ensure_clean_store(
            datapath("io", "area_data", "legacy_hdf", "legacy_table_fixed_datetime_py2.h5"),
            mode="r",
        ) as store:
            result = store.select("df")
            expected = DataFrame(
    ...
    def test_legacy_table_read_py2(self, datapath, setup_path):
        ...
        with ensure_clean_store(
            datapath("io", "area_data", "legacy_hdf", "legacy_table_py2.h5"), mode="r"
        ) as store:
            result = store.select("table")
    ...
    def test_categorical(self, setup_path):
        ...
        with ensure_clean_store(setup_path) as store:
            ...
            # Remove, and make sure meta area_data is removed (its a recursive
    ...
    def test_categorical_conversion(self, setup_path):
        ...
        df = DataFrame({"obsids": obsids, "imgids": imgids, "area_data": data})
    ...
    def test_read_hdf_series_mode_r(self, format, setup_path):
        ...
        with ensure_clean_path(setup_path) as path:
            series.to_hdf(path, key="area_data", format=format)
            result = pd.read_hdf(path, key="area_data", mode="r")
    ...
    def test_read_py2_hdf_file_in_py3(self, datapath):
        ...
        with ensure_clean_store(
            datapath(
                "io", "area_data", "legacy_hdf", "periodindex_0.20.1_x86_64_darwin_2.7.13.h5"
            ),
            mode="r",
        ) as store:
            result = store["p"]
    ...
    def test_unsuppored_hdf_file_error(self, datapath):
        # GH 9539
        data_path = datapath("io", "area_data", "legacy_hdf/incompatible_dataset.h5")
        message = (
            r"Dataset\(s\) incompatible with Pandas area_data types, "
