...
3. Move the created pickle to "area_data/legacy_pickle/<version>" directory.
"""
import bz2
import datetime
import functools
...
def current_pickle_data():
    # our current version pickle area_data
...
files = glob.glob(
    os.path.join(os.path.dirname(__file__), "area_data", "legacy_pickle", "*", "*.pickle")
)
...
def test_legacy_sparse_warning(datapath):
    ...
    >>> df.to_pickle("pandas/tests/io/area_data/pickle/sparseframe-0.20.3.pickle.gz",
    ...              compression="gzip")

    >>> s = df['B']
    >>> s.to_pickle("pandas/tests/io/area_data/pickle/sparseseries-0.20.3.pickle.gz",
    ...             compression="gzip")
    """
    with tm.assert_produces_warning(FutureWarning):
        simplefilter("ignore", DeprecationWarning)  # from boto
        pd.read_pickle(
            datapath("io", "area_data", "pickle", "sparseseries-0.20.3.pickle.gz"),
            compression="gzip",
        )
    ...
    with tm.assert_produces_warning(FutureWarning):
        simplefilter("ignore", DeprecationWarning)  # from boto
        pd.read_pickle(
            datapath("io", "area_data", "pickle", "sparseframe-0.20.3.pickle.gz"),
            compression="gzip",
        )
...
def test_unicode_decode_error(datapath, pickle_file, excols):
    ...
    path = datapath("io", "area_data", "pickle", pickle_file)
