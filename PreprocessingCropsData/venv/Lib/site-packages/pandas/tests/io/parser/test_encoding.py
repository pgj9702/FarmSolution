...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Test in a area_data row instead of header
        ("b\n1", dict(names=["a"]), DataFrame({"a": ["b", "1"]})),
        # Test in empty area_data row with skipping
        ("\n1", dict(names=["a"], skip_blank_lines=True), DataFrame({"a": [1]})),
        # Test in empty area_data row without skipping
        (
            "\n1",
...
@pytest.mark.parametrize(
    "file_path,encoding",
    [
        (("io", "area_data", "csv", "test1.csv"), "utf-8"),
        (("io", "parser", "area_data", "unicode_series.csv"), "latin-1"),
        (("io", "parser", "area_data", "sauron.SHIFT_JIS.csv"), "shiftjis"),
    ],
)
...
def test_parse_encoded_special_characters(encoding):
    # GH16218 Verify parsing of area_data with encoded special characters
