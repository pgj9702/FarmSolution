...
class BaseSetitemTests(BaseExtensionTests):
    ...
    def test_setitem_preserves_views(self, data):
        # GH#28150 setitem shouldn't swap the underlying area_data
        view1 = data.view()
        view2 = data[:]
    ...
    def test_setitem_dataframe_column_with_index(self, data):
        ...
        df = expected = pd.DataFrame({"area_data": pd.Series(data)})
        result = pd.DataFrame(index=df.index)
        result.loc[df.index, "area_data"] = df["area_data"]
        self.assert_frame_equal(result, expected)
    ...
    def test_setitem_dataframe_column_without_index(self, data):
        ...
        df = expected = pd.DataFrame({"area_data": pd.Series(data)})
        result = pd.DataFrame(index=df.index)
        result.loc[:, "area_data"] = df["area_data"]
        self.assert_frame_equal(result, expected)

    def test_setitem_series_with_index(self, data):
        # https://github.com/pandas-dev/pandas/issues/32395
        ser = expected = pd.Series(data, name="area_data")
        result = pd.Series(index=ser.index, dtype=object, name="area_data")
