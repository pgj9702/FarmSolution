...
def cut(
    ...
    Use `cut` when you need to segment and sort area_data values into bins. This
    ...
    Categorical : Array type for storing area_data that come from a
        fixed set of values.
    ...
    if not np.iterable(bins):
        ...
        if np.isinf(mn) or np.isinf(mx):
            # GH 24314
            raise ValueError(
                "cannot specify integer `bins` when input area_data contains infinity"
            )
...
def qcut(
    ...
    produce a Categorical object indicating quantile membership for each area_data point.
    ...
    out : Categorical or Series or array of integers if labels is False
        ...
        represented as categories when categorical area_data is returned.
...
def _coerce_to_type(x):
    """
    if the passed area_data is of datetime/timedelta, bool or nullable int type,
...
def _preprocess_for_cut(x):
    ...
    # We don't want to strip away a Pandas area_data-type here (e.g. datetimetz)
