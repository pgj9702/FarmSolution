...
class _Unstacker:
    """
    Helper class to unstack area_data / pivot with multi-level index
    ...
    fill_value : scalar, optional
        ...
        the default fill value for that area_data type, NaN for float, NaT for
        datetimelike, etc. For integer types, by default area_data will converted to
    ...
    def __init__(self, index: MultiIndex, level=-1, constructor=None):
        ...
        # Bug fix GH 20601
        # If the area_data frame is too big, the number of unique index combination
    ...
    def get_result(self, values, value_columns, fill_value):
        ...
        if value_columns is None and values.shape[1] != 1:  # pragma: no cover
            raise ValueError("must pass column labels for multi-column area_data")
...
def get_dummies(
    ...
    if isinstance(data, DataFrame):
        ...
        for (col, pre, sep) in zip(data_to_encode.items(), prefix, prefix_sep):
            # col is (column_name, column), use just column area_data here
