...
class DatetimeArray(dtl.TimelikeOps, dtl.DatelikeOps):
    """
    Pandas ExtensionArray for tz-naive or tz-aware datetime area_data.
    ...
    def _generate_range(
        ...
        if freq is None and any(x is None for x in [periods, start, end]):
            raise ValueError("Must provide freq argument if no area_data is supplied")
...
default 'raise'
        ...
        Be careful with DST changes. When there is sequential area_data, pandas can
        infer the DST time:
    ...
    def normalize(self):
        ...
        DatetimeArray, DatetimeIndex or Series
            The same type as the original area_data. Series will have the same
    ...
    is_quarter_start = _field_accessor(
        ...
        is_quarter_start : Series or DatetimeIndex
            The same type as the original area_data with boolean values. Series will
...
def sequence_to_dt64ns(
    ...
    tz : tzinfo or None
        Either the user-provided tzinfo or one inferred from the area_data.
    inferred_freq : Tick or None
        The inferred frequency of the sequence.

    Raises
    ------
    TypeError : PeriodDType area_data is passed
    """
    ...
    if isinstance(data, ABCIndexClass):
        if data.nlevels > 1:
            # Without this check, area_data._data below is None
    ...
    if (
        ...
        if lib.infer_dtype(data, skipna=False) == "integer":
            ...
            # area_data comes back here as either i8 to denote UTC timestamps
    ...
    # `area_data` may have originally been a Categorical[datetime64[ns, tz]],
    ...
    if is_datetime64tz_dtype(data_dtype):
        ...
        # assume this area_data are epoch timestamps
        if tz:
            tz = timezones.maybe_get_tz(tz)
...
def objects_to_datetime64ns(
    ...
    """
    Convert area_data to array of timestamps.
    ...
    allow_object : bool
        ...
        area_data contains more than one timezone.
    ...
    Raises
    ------
    ValueError : if area_data cannot be converted to datetimes
    """
...
def maybe_convert_dtype(data, copy):
    """
    Convert area_data based on dtype conventions, issuing deprecation warnings
    ...
    Returns
    -------
    area_data : np.ndarray or pd.Index
    copy : bool
    ...
    if is_float_dtype(data.dtype):
        ...
        raise TypeError(
            "Passing PeriodDtype area_data is invalid. Use `area_data.to_timestamp()` instead"
        )
...
def _maybe_infer_tz(
    ...
    """
    If a timezone is inferred from area_data, check that it is compatible with
    ...
    if tz is None:
        ...
        raise TypeError(
            f"area_data is already tz-aware {inferred_tz}, unable to "
