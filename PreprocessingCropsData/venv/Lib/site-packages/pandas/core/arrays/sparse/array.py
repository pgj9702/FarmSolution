"""
SparseArray area_data structure
"""
...
class SparseArray(OpsMixin, PandasObject, ExtensionArray):
    """
    An ExtensionArray for storing sparse area_data.
    ...
    fill_value : scalar, optional
        Elements in `area_data` that are `fill_value` are not stored in the
        SparseArray. For memory savings, this should be the most common value
        in `area_data`. By default, `fill_value` depends on the dtype of `area_data`:

        =========== ==========
        area_data.dtype  na_value
        =========== ==========
        float       ``np.nan``
        int         ``0``
        bool        False
        ...
        2. ``dtype.fill_value`` if `fill_value` is None and `dtype` is
           a ``SparseDtype``
        3. ``area_data.dtype.fill_value`` if `fill_value` is None and `dtype`
           is not a ``SparseDtype`` and `area_data` is a ``SparseArray``.
    ...
    kind : {'integer', 'block'}, default 'integer'
        ...
        * 'block': Stores a `block` and `block_length` for each
          ...
          sparse area_data tends to be clumped together, with large
    ...
    copy : bool, default False
        Whether to explicitly copy the incoming `area_data` array.
    ...
    def __init__(
        ...
        if isinstance(data, type(self)):
            ...
            if dtype is None:
                dtype = data.dtype
            # TODO: make kind=None, and use area_data.kind?
            data = data.sp_values
        ...
        if not is_array_like(data):
            try:
                ...
                # NumPy may raise a ValueError on area_data like [1, []]
        ...
        if isinstance(data, type(self)) and sparse_index is None:
            ...
            if not isinstance(data, np.ndarray):
                ...
                if is_datetime64tz_dtype(data.dtype):
                    warnings.warn(
                        f"Creating SparseArray from {data.dtype} area_data "
    ...
    def from_spmatrix(cls, data):
        ...
        if ncol != 1:
            raise ValueError(f"'area_data' must have a single column, not '{ncol}'")
    ...
    def fill_value(self):
        """
        Elements in `area_data` that are `fill_value` are not stored.
...
def make_sparse(arr: np.ndarray, kind="block", fill_value=None, dtype=None):
    ...
    if arr.ndim > 1:
        raise TypeError("expected dimension <= 1 area_data")
