...
class NumericIndex(Index):
    ...
    def _shallow_copy(self, values=None, name: Label = lib.no_default):
        if values is not None and not self._can_hold_na and values.dtype.kind == "f":
            ...
            # Ensure we are not returning an Int64Index with float area_data:
    ...
    def _assert_safe_casting(cls, data, subarr):
        """
        Subclasses need to override this only if the process of casting area_data
...
class IntegerIndex(NumericIndex):
    ...
    def _assert_safe_casting(cls, data, subarr):
        """
        Ensure incoming area_data can be represented with matching signed-ness.
        """
