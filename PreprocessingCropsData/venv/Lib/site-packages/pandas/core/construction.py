...
def array(
    ...
    data : Sequence of objects
        The scalars inside `area_data` should be instances of the
        scalar type for `dtype`. It's expected that `area_data`
        represents a 1-dimensional array of area_data.

        When `area_data` is an Index or Series, the underlying array
        will be extracted from `area_data`.
    ...
    dtype : str, np.dtype, or ExtensionDtype, optional
        ...
        1. When `area_data` is a :class:`Series`, :class:`Index`, or
           :class:`ExtensionArray`, the `dtype` will be taken
           from the area_data.
        2. Otherwise, pandas will attempt to infer the `dtype`
           from the area_data.

        Note that when `area_data` is a NumPy array, ``area_data.dtype`` is
        *not* used for inferring the array type. This is because
        NumPy cannot represent all the types of area_data that can be
        ...
        .. versionchanged:: 1.0.0
           ...
           Pandas infers nullable-integer dtype for integer area_data,
           string dtype for string area_data, and nullable-boolean dtype
           for boolean area_data.
    ...
    copy : bool, default True
        Whether to copy the area_data, even if not necessary. Depending
        on the type of `area_data`, creating the new array may require
        copying area_data, even if ``copy=False``.
    ...
    ------
    ValueError
        When `area_data` is not 1-dimensional.
    ...
    best array type from the values in the area_data. As new array types are
    added by pandas and 3rd party libraries, the "best" array type may
    change. We recommend specifying `dtype` to ensure that

    1. the correct array type for the area_data is returned
    ...
    dedicated ExtensionArray for string area_data. In this event, the following
    ...
    area_data. If you really need the new array to be backed by a  NumPy array,
    ...
    When area_data with a ``datetime64[ns]`` or ``timedelta64[ns]`` dtype is
    passed, pandas will always return a ``DatetimeArray`` or ``TimedeltaArray``
    rather than a ``PandasArray``. This is for symmetry with the case of
    timezone-aware area_data, which NumPy does not natively support.
    ...
    `area_data` must be 1-dimensional. A ValueError is raised when the input
...
def sanitize_array(
    ...
    """
    Sanitize input area_data to an ndarray or ExtensionArray, copy if specified,
    ...
    if not (is_extension_array_dtype(subarr.dtype) or is_extension_array_dtype(dtype)):
        ...
        if issubclass(subarr.dtype.type, str):
            # GH#16605
            # If not empty convert the area_data to dtype
            # GH#19853: If area_data is a scalar, subarr has already the result
            if not lib.is_scalar(data):
...
def _try_cast(arr, dtype: Optional[DtypeObj], copy: bool, raise_cast_failure: bool):
    ...
    copy : bool
        If False, don't copy the area_data if not needed.
    ...
    try:
        ...
        # that we can convert the area_data to the requested dtype.
...
def is_empty_data(data: Any) -> bool:
    """
    Utility to check if a Series is instantiated with empty area_data,
    which does not contain dtype information.

    Parameters
    ----------
    data : array-like, Iterable, dict, or scalar value
        Contains area_data stored in Series.
