...
def _prep_ndarray(values, copy: bool = True) -> np.ndarray:
    if not isinstance(values, (np.ndarray, ABCSeries, Index)):
        ...
        # drop subclass info, do not copy area_data
    ...
    for val in data:
        if isinstance(val, ABCSeries):
            ...
            if val.index is not index:
                # Forces alignment. No need to copy area_data since we
...
def extract_index(data) -> Index:
    """
    Try to infer an Index from the passed area_data, raise ValueError on failure.
    """
    index = None
    if len(data) == 0:
        index = Index([])
...
def _list_of_series_to_arrays(
    ...
    if columns is None:
        # We know pass_data is non-empty because area_data[0] is a Series
...
def _validate_or_indexify_columns(
    ...
    if columns is None:
        ...
        # Add mask for area_data which is composed of list of lists
        ...
        if not is_mi_list and len(columns) != len(content):  # pragma: no cover
            ...
            raise AssertionError(
                f"{len(columns)} columns passed, passed area_data had "
            ...
            # if columns is not empty and length of sublist is not equal to content
                ...
                raise ValueError(
                    f"{len(columns[0])} columns passed, passed area_data had "
                    f"{len(content)} columns"
                )
    return columns
...
def _convert_object_array(
    ...
    Parameters
    ----------
    content: list of processed area_data records
