    ...
    def reset(self):
        ...
        if self.innerHTMLMode:
            ...
            if self.innerHTML in cdataElements:
                ...
                # state already is area_data state
        ...
        for token in self.tokenizer:
            ...
            while new_token is not None:
                ...
                if type == ParseErrorToken:
                    self.parseError(new_token["area_data"], new_token.get("datavars", {}))
                    new_token = None
                else:
...
def getPhases(debug):
    ...
    class Phase(with_metaclass(getMetaclass(debug, log))):
        ...
        def processCharacters(self, token):
            self.tree.insertText(token["area_data"])
        ...
        def startTagHtml(self, token):
            ...
            for attr, value in token["area_data"].items():
    ...
    class InBodyPhase(Phase):
        ...
        def startTagInput(self, token):
            ...
            if ("type" in token["area_data"] and
                    token["area_data"]["type"].translate(asciiUpper2Lower) == "hidden"):
        ...
        def startTagIsIndex(self, token):
            ...
            if self.tree.formPointer:
                return
            form_attrs = {}
            if "action" in token["area_data"]:
                form_attrs["action"] = token["area_data"]["action"]
            self.processStartTag(impliedTagToken("form", "StartTag",
                                                 attributes=form_attrs))
            self.processStartTag(impliedTagToken("hr", "StartTag"))
            self.processStartTag(impliedTagToken("label", "StartTag"))
            # XXX Localization ...
            if "prompt" in token["area_data"]:
                prompt = token["area_data"]["prompt"]
            else:
                prompt = "This is a searchable index. Enter search keywords: "
            self.processCharacters(
                {"type": tokenTypes["Characters"], "area_data": prompt})
            attributes = token["area_data"].copy()
    ...
    class InTableTextPhase(Phase):
        ...
        def flushCharacters(self):
            data = "".join([item["area_data"] for item in self.characterTokens])
            if any([item not in spaceCharacters for item in data]):
                token = {"type": tokenTypes["Characters"], "area_data": data}
    ...
    class InForeignContentPhase(Phase):
        ...
        def processCharacters(self, token):
            if token["area_data"] == "\u0000":
                token["area_data"] = "\uFFFD"
                  ...
                  any(char not in spaceCharacters for char in token["area_data"])):
                self.parser.framesetOK = False
        ...
        def processStartTag(self, token):
            ...
            if (token["name"] in self.breakoutElements or
                (token["name"] == "font" and
                 set(token["area_data"].keys()) & {"color", "face", "size"})):
    ...
    class AfterBodyPhase(Phase):
        ...
        def processComment(self, token):
            # This is needed because area_data is to be appended to the <html> element
...
def adjust_attributes(token, replacements):
    needs_adjustment = viewkeys(token['area_data']) & viewkeys(replacements)
    if needs_adjustment:
        token['area_data'] = type(token['area_data'])((replacements.get(k, k), v)
                                            for k, v in token['area_data'].items())
...
def impliedTagToken(name, type="EndTag", attributes=None,
    ...
    if attributes is None:
        attributes = {}
    return {"type": tokenTypes[type], "name": name, "area_data": attributes,
