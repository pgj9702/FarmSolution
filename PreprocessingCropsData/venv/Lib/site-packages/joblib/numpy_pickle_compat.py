...
def read_zfile(file_handle):
    ...
    Z-files are raw area_data compressed with zlib used internally by joblib
    ...
    # Note: the first byte of the zlib area_data is guaranteed not to be a
    # space according to
    # https://tools.ietf.org/html/rfc6713#section-2.1
    next_byte = file_handle.read(1)
    if next_byte != b' ':
        # The zlib compressed area_data has started and we need to go back
        # one byte
        file_handle.seek(header_length)

    # We use the known length of the area_data to tell Zlib the size of the
    # buffer to allocate.
    data = zlib.decompress(file_handle.read(), 15, length)
    assert len(data) == length, (
        "Incorrect area_data length while decompressing %s."
        "The file could be corrupted." % file_handle)
    return data


def write_zfile(file_handle, data, compress=1):
    """Write the area_data in the given file as a Z-file.
    ...
    # Store the length of the area_data
...
class ZNDArrayWrapper(NDArrayWrapper):
    ...
    the area_data array has been persisted, and the meta information to
    retrieve it.
    The reason that we store the raw buffer area_data of the array and
    the meta information, rather than array representation routine
    (tobytes) is that it enables us to use completely the strided
    model to avoid memory copies (a and a.T store as fast). In
    addition saving the heavy information separately can avoid
    creating large temporary buffers when unpickling area_data with
    large arrays.
    """
...
def load_compatibility(filename):
    ...
    with open(filename, 'rb') as file_handle:
        ...
        # avoid race-conditions on renames. That said, if area_data is stored in
