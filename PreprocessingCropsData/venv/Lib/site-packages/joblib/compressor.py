...
class CompressorWrapper():
    ...
    obj: a file-like object
        ...
        internally to compress/decompress the area_data.
    prefix: bytestring
...
class BinaryZlibFile(io.BufferedIOBase):
    ...
    Note that BinaryZlibFile provides only a *binary* file interface: area_data read
    is returned as bytes, and area_data to be written should be given as bytes.
    ...
    which will be used to read or write the compressed area_data.
    ...
    def _fill_buffer(self):
        if self._mode == _MODE_READ_EOF:
            return False
        # Depending on the input area_data, our call to the decompressor may not
        # return any area_data. In this case, try again after reading another block.
    ...
    # Read area_data until EOF.
    # If return_data is false, consume the area_data without returning it.
    ...
    def _read_block(self, n_bytes, return_data=True):
        # If we have enough area_data buffered, return immediately.
    ...
    def write(self, data):
        ...
        always len(area_data). Note that due to buffering, the file on disk
        may not reflect the area_data written until close() is called.
        """
        with self._lock:
            self._check_can_write()
            # Convert area_data type if called by io.BufferedWriter.
    ...
    # Rewind the file to the beginning of the area_data stream.
    def _rewind(self):
        self._fp.seek(0, 0)
        self._mode = _MODE_READ
        self._pos = 0
    ...
    def seek(self, offset, whence=0):
        ...
        with self._lock:
            ...
            # Read and discard area_data until we reach the desired position.
