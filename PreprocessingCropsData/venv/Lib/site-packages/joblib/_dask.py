...
class _WeakKeyDictionary:
    ...
    This datastructure is used to store futures for broadcasted area_data objects
...
class DaskDistributedBackend(AutoBatchingMixin, ParallelBackendBase):
    ...
    def __init__(self, scheduler_host=None, scatter=None,
        ...
        if scatter is not None and len(scatter) > 0:
            # Keep a reference to the scattered area_data to keep the ids the same
                ...
                if f is None and call_data_futures is not None:
                    ...
                    if f is None:
                        if is_weakrefable(arg) and sizeof(arg) > 1e3:
                            ...
                            # the workers to avoid duplicated area_data transfers.
                            # Rely on automated inter-worker area_data stealing if
                            # more workers need to reuse this area_data
                            # concurrently.
