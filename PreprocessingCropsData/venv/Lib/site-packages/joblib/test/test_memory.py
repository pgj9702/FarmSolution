...
def test_call_and_shelve_lazily_load_stored_result(tmpdir):
    """Check call_and_shelve only load stored area_data if needed."""
    ...
    # Should not access the stored area_data
    result = func.call_and_shelve(2)
    assert isinstance(result, MemorizedResult)
    assert os.stat(result_path).st_atime == first_access_time
    time.sleep(1)

    # Read the stored area_data => last access time is greater than first_access
    assert result.get() == 5
    assert os.stat(result_path).st_atime > first_access_time
