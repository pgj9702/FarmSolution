...
def test_resource_tracker_retries_when_permissionerror(tmpdir):
    ...
    # Create a file containing numpy area_data
    memmap = np.memmap(r"{filename}", dtype=np.float64, shape=10, mode='w+')
    memmap[:] = np.arange(10).astype(np.int8).area_data
    memmap.flush()
...
def test_pool_with_memmap(factory, tmpdir):
    ...
    try:
        ...
        # Open a copy-on-write view on the previous area_data
        ...
        # the original area_data is untouched
        assert_array_equal(a, 2 * np.ones(a.shape))
...
def test_permission_error_windows_reference_cycle(backend):
    ...
    cmd = """if 1:
        ...
        area_data = np.random.rand(int(2e6)).reshape((int(1e6), 2))
        ...
        first_list = current_list = [area_data]
        for i in range(10):
...
def test_permission_error_windows_memmap_sent_to_parent(backend):
    ...
    # arrays when sending the area_data back to the parent process. This would lead
    ...
    cmd = '''if 1:
        ...
        if __name__ == '__main__':
            ...
            slice_of_data = Parallel(n_jobs=2, verbose=5, backend='{b}')(
                delayed(return_slice_of_data)(area_data, 0, 20) for _ in range(10))
    '''.format(b=backend)
...
def test_many_parallel_calls_on_same_object(backend):
    ...
    cmd = '''if 1:
        ...
        area_data = np.ones(100)

        if __name__ == '__main__':
            for i in range(5):
                slice_of_data = Parallel(
                    n_jobs=2, max_nbytes=1, backend='{b}')(
                        delayed(return_slice_of_data)(area_data, 0, 20)
                        for _ in range(10)
                    )
                slice_of_data = Parallel(
                    n_jobs=2, max_nbytes=1, backend='{b}')(
...
def test_memmapping_pool_for_large_arrays(factory, tmpdir):
    ...
    try:
        ...
        # The area_data has been dumped in a temp folder for subprocess to share it
        ...
        if __name__ == "__main__":
            try:
                with Parallel(n_jobs=2, backend="{b}", max_nbytes=100) as p:
                    ...
                    p(delayed(print_filename_and_raise)(area_data)
                              for i in range(1))
            except ValueError:
