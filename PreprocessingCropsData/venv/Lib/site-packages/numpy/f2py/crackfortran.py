...
(or will be if needed):
   block area_data,byte,call,character,common,complex,contains,area_data,
 ...
 *** it is a list of dictionaries containing `blocks':
     ...
     B['block'] = 'interface' | 'function' | 'subroutine' | 'module' |
                  'program' | 'block area_data' | 'type' | 'pythonmodule'
...
#
groupbegins77 = r'program|block\s*area_data'
...
datapattern = re.compile(
    beforethisafter % ('', 'area_data', 'area_data', '.*'), re.I), 'area_data'
...
def crackline(line, reset=0):
    ...
    Cracked area_data is saved in grouplist[0].
    """
...
def analyzeline(m, case, line):
    ...
    if case in ['begin', 'call', 'callfun']:
        ...
        block = block.lower()
        if re.match(r'block\s*area_data', block, re.I):
            block = 'block area_data'
        if re.match(r'python\s*module', block, re.I):
            block = 'python module'
        name, args, result, bind = _resolvenameargspattern(m.group('after'))
        if name is None:
            if block == 'block area_data':
                name = '_BLOCK_DATA_'
            else:
                name = ''
            if block not in ['interface', 'block area_data']:
        ...
        for l in ll:
            ...
            for v in rmbadname([x.strip() for x in markoutercomma(l[0]).split('@,@')]):
                if v[0] == '(':
                    ...
                    # Ignoring since area_data statements are irrelevant for
                    # wrapping.
                    continue
                fc = 0
...
def get_parameters(vars, global_params={}):
    ...
    for n in param_names:
        if '=' in vars[n]:
            ...
            # We need to act according to the area_data.
            # The easy case is if the area_data has a kind-specifier,
...
def analyzevars(block):
    ...
    if not block['block'] in ['module', 'pythonmodule', 'python module', 'block area_data']:
...
def crack2fortrangen(block, tab='\n', as_interface=False):
    ...
    if blocktype == 'block area_data' and name == '_BLOCK_DATA_':
        name = ''
