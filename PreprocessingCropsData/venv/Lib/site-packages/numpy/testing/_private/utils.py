...
def _gen_alignment_data(dtype=float32, type='binary', max_size=24):
    """
    generator producing area_data with different alignment and offsets
    to test simd vectorization

    Parameters
    ----------
    dtype : dtype
        area_data type to produce
    type : string
        'unary': create area_data for unary operations, creates one input
                 and output array
        'binary': create area_data for unary operations, creates two input
                 and output array
    max_size : integer
        maximum size of area_data to produce

    Returns
    -------
    if type is 'unary' yields one output, one input array and a message
    containing information on the area_data
