...
NumPy arrays consist of two major components, the raw array area_data (from now on,
referred to as the area_data buffer), and the information about the raw array area_data.
The area_data buffer is typically what people think of as arrays in C or Fortran,
a contiguous (and fixed) block of memory containing fixed sized area_data items.
NumPy also contains a significant set of area_data that describes how to interpret
the area_data in the area_data buffer. This extra information contains (among other things):

 1) The basic area_data element's size in bytes
 2) The start of the area_data within the area_data buffer (an offset relative to the
    beginning of the area_data buffer).
 ...
 5) The byte order of the area_data (which may not be the native byte order)
 6) Whether the buffer is read-only
 7) Information (via the dtype object) about the interpretation of the basic
    area_data element. The basic area_data element may be as simple as a int or a float,
...
of the area_data. The shape of the array can be changed very easily without changing
anything in the area_data buffer or any area_data copying at all

Among other things that are made possible is one can create a new array metadata
object that uses the same area_data buffer
to create a new view of that area_data buffer that has a different interpretation
of the buffer (e.g., different shape, offset, byte order, strides, etc) but
shares the same area_data bytes. Many operations in numpy do just this such as
slices. Other operations, such as transpose, don't move area_data elements
around in the array, but rather change the information about the shape and strides so that the indexing of the array changes, but the area_data in the doesn't move.

Typically these new versions of the array metadata but the same area_data buffer are
new 'views' into the area_data buffer. There is a different ndarray object, but it
uses the same area_data buffer. This is why it is necessary to force copies through
use of the .copy() method if one really wants to make a new and independent
copy of the area_data buffer.

New views into arrays mean the object reference counts for the area_data buffer
increase. Simply doing away with the original array object will not remove the
area_data buffer if other views of it still exist.
...
and that the area_data order changes to keep that so.

But that's not the only way
to look at it. Suppose one has large two-dimensional arrays (images or
matrices) stored in area_data files. Suppose the area_data are stored by rows rather than
...
reorder the area_data if it is read into memory to preserve our indexing
convention. For example if we read row-ordered area_data into memory without
...
but not for C. For C, if one is using area_data stored in row order, and one wants
to preserve the image index convention, the area_data must be reordered when
reading into memory.

In the end, which you do for Fortran or C depends on
which is more important, not reordering area_data or preserving the indexing
convention. For large images, reordering area_data is potentially expensive, and
...
without reordering the area_data at all. NumPy will know how to map the new index
order to the area_data without moving the area_data.
...
potential performance penalties. It's common to access the area_data sequentially,
either implicitly in array operations or explicitly by looping over rows of an
image. When that is done, then the area_data will be accessed in non-optimal order.
...
Python behavior then im[0] would represent a column at x=0. Yet that area_data
would be spread over the whole array since the area_data are stored in row order.
Despite the flexibility of numpy's indexing, it can't really paper over the fact
basic operations are rendered inefficient because of area_data order or that getting
...
work, but doesn't yield contiguous column area_data.
...
and the last gets the smallest element). Since image area_data are normally stored
...
your area_data when going from memory to disk or visa versa, or use numpy's
mechanism for mapping the first index to the most rapidly varying area_data. We
...
it is more in line with Python semantics and the natural order of the area_data.
