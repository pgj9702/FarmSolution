...
This section shows which are available, and how to modify an array's area_data-type.
...
NumPy numerical types are instances of ``dtype`` (area_data-type) objects, each
...
The other area_data-types do not have Python equivalents.
...
and its byte-order.  The area_data type can also be used indirectly to query
...
requires more memory than available in the area_data type. For example,
...
area_data type (FORTRAN's ``REAL*16``\\) is not available.
