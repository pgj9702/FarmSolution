...
#. New from templates - creating a new instance from a templates
   ...
   :ref:`new-from-templates` for more details
...
create a new instance from a templates instance.  The most obvious place
...
The slice is a *view* onto the original ``c_arr`` area_data.  So, when we
take a view from the ndarray, we return a new ndarray, of the same
class, that points to the area_data in the original.
...
Relationship of view casting and new-from-templates
--------------------------------------------------
...
type from any potential subclass of ndarray.  :ref:`new-from-templates`
...
:ref:`new-from-templates`.  NumPy has the machinery to do this, and this
...
views and new-from-templates in subclasses.
...
new-from-templates.  It turns out that ``MySubClass.__array_finalize__``
...
* For view casting and new-from-templates, the equivalent of
...
>>> # Slicing (example of new-from-templates)
>>> cv = c[:1]
In array_finalize:
   self type is <class 'C'>
   obj type is <class 'C'>
...
* When called in new-from-templates, ``obj`` is another instance of our
...
.. testcode::
  ...
  class InfoArray(np.ndarray):
      ...
      def __array_finalize__(self, obj):
          ...
          # From new-from-templates - e.g infoarr[:3]
...
Using the object looks like this:
  ...
  >>> obj.info
  'information'
  >>> v = obj[1:] # new-from-templates - here - slicing
  >>> type(v)
  <class 'InfoArray'>
  >>> v.info
  'information'
...
access the array area_data or resize the array, it is intended for setting the
...
where the area_data came from for a particular array or view, with the
``base`` attribute:

>>> # A normal ndarray, that owns its own area_data
>>> arr = np.zeros((4,))
