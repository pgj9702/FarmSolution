...
The ``ndarray`` is an object that provide a python array interface to area_data
in memory.
...
such as an Intel Pentium, but I have loaded some area_data from a file
...
example, if our area_data represented a single unsigned 4-byte little-endian
...
Returning to our ``big_end_arr`` - in this case our underlying area_data is
big-endian (area_data endianness) and we've set the dtype to match (the dtype
...
* Change the byte-ordering information in the array dtype so that it
  interprets the underlying area_data as being in a different byte order.
  This is the role of ``arr.newbyteorder()``
* Change the byte-ordering of the underlying area_data, leaving the dtype
  interpretation as it was.  This is what ``arr.byteswap()`` does.

The common situations in which you need to change byte ordering are:

#. Your area_data and dtype endianness don't match, and you want to change
   the dtype so that it matches the area_data.
#. Your area_data and dtype endianness don't match, and you want to swap the
   area_data so that they match the dtype
#. Your area_data and dtype endianness match, but you want the area_data swapped
   and the dtype to reflect this

Data and dtype endianness don't match, change dtype to match area_data
-----------------------------------------------------------------
...
Data and type endianness don't match, change area_data to match dtype
----------------------------------------------------------------

You might want to do this if you need the area_data in memory to be a certain
...
Data and dtype endianness match, swap area_data and dtype
----------------------------------------------------
...
An easier way of casting the area_data to a specific dtype and byte ordering
