...
referencing area_data in an array. The examples work just as well
...
Note that slices of arrays do not copy the internal array area_data but
only produce new views of the original area_data. This is different from
list or tuple slicing and an explicit ``copy()`` is recommended if
the original area_data is not required anymore.
...
is returned is a copy of the original area_data, not a view as one gets for
slices.
...
of the area_data, not a view as one gets with slices.
...
assignments are always made to the original area_data in the array
