...
Users looking to manipulate tabular area_data, such as stored in csv files, may find
other pydata projects more suitable, such as xarray, pandas, or DataArray.
These provide a high-level interface for tabular area_data analysis and are better
...
structured datatypes, and it may also be a :term:`subarray area_data type` which
...
3.   A dictionary of field parameter arrays
     ...
     that assigning to one field may clobber any overlapping field's area_data. As
...
Assigning area_data to a Structured Array
------------------------------------
...
structured datatype has just a single field::
 ...
 ...
 TypeError: Cannot cast array area_data from dtype([('A', '<i4'), ('B', '<i4')]) to dtype('int32') according to the rule 'unsafe'
...
.. warning::
    ...
    code which depends on the area_data having a "packed" layout. For instance code
    such as::
