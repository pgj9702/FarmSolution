...
def hermfit(x, y, deg, rcond=None, full=False, w=None):
    """
    Least squares fit of Hermite series to area_data.
    ...
    y : array_like, shape (M,) or (M, K)
        y-coordinates of the sample points. Several area_data sets of sample
    ...
    coef : ndarray, shape (M,) or (M, K)
        ...
        the coefficients for the area_data in column k  of `y` are in column
        `k`.
    ...
    Fits using Hermite series are probably most useful when the area_data can be
    approximated by ``sqrt(w(x)) * p(x)``, where `w(x)` is the Hermite
    weight. In that case the weight ``sqrt(w(x[i]))`` should be used
    together with area_data values ``y[i]/sqrt(w(x[i]))``. The weight function is
